<?php


namespace Firesphere\SearchConfig\Helpers;

/**
 * Class Synonyms
 * Source: @link https://raw.githubusercontent.com/heiswayi/spelling-uk-vs-us/
 * @package Firesphere\SearchConfig\Helpers
 */
class Synonyms
{

    public static $uk_us = [
        0    =>
            [
                0 => 'accessorise',
                1 => 'accessorize',
            ],
        1    =>
            [
                0 => 'accessorised',
                1 => 'accessorized',
            ],
        2    =>
            [
                0 => 'accessorises',
                1 => 'accessorizes',
            ],
        3    =>
            [
                0 => 'accessorising',
                1 => 'accessorizing',
            ],
        4    =>
            [
                0 => 'acclimatisation',
                1 => 'acclimatization',
            ],
        5    =>
            [
                0 => 'acclimatise',
                1 => 'acclimatize',
            ],
        6    =>
            [
                0 => 'acclimatised',
                1 => 'acclimatized',
            ],
        7    =>
            [
                0 => 'acclimatises',
                1 => 'acclimatizes',
            ],
        8    =>
            [
                0 => 'acclimatising',
                1 => 'acclimatizing',
            ],
        9    =>
            [
                0 => 'accoutrements',
                1 => 'accouterments',
            ],
        10   =>
            [
                0 => 'aeon',
                1 => 'eon',
            ],
        11   =>
            [
                0 => 'aeons',
                1 => 'eons',
            ],
        12   =>
            [
                0 => 'aerogramme',
                1 => 'aerogram',
            ],
        13   =>
            [
                0 => 'aerogrammes',
                1 => 'aerograms',
            ],
        14   =>
            [
                0 => 'aeroplane',
                1 => 'airplane',
            ],
        15   =>
            [
                0 => 'aeroplanes ',
                1 => 'airplanes ',
            ],
        16   =>
            [
                0 => 'aesthete',
                1 => 'esthete',
            ],
        17   =>
            [
                0 => 'aesthetes',
                1 => 'esthetes',
            ],
        18   =>
            [
                0 => 'aesthetic',
                1 => 'esthetic',
            ],
        19   =>
            [
                0 => 'aesthetically',
                1 => 'esthetically',
            ],
        20   =>
            [
                0 => 'aesthetics',
                1 => 'esthetics',
            ],
        21   =>
            [
                0 => 'aetiology',
                1 => 'etiology',
            ],
        22   =>
            [
                0 => 'ageing',
                1 => 'aging',
            ],
        23   =>
            [
                0 => 'aggrandisement',
                1 => 'aggrandizement',
            ],
        24   =>
            [
                0 => 'agonise',
                1 => 'agonize',
            ],
        25   =>
            [
                0 => 'agonised',
                1 => 'agonized',
            ],
        26   =>
            [
                0 => 'agonises',
                1 => 'agonizes',
            ],
        27   =>
            [
                0 => 'agonising',
                1 => 'agonizing',
            ],
        28   =>
            [
                0 => 'agonisingly',
                1 => 'agonizingly',
            ],
        29   =>
            [
                0 => 'almanack',
                1 => 'almanac',
            ],
        30   =>
            [
                0 => 'almanacks',
                1 => 'almanacs',
            ],
        31   =>
            [
                0 => 'aluminium',
                1 => 'aluminum',
            ],
        32   =>
            [
                0 => 'amortisable',
                1 => 'amortizable',
            ],
        33   =>
            [
                0 => 'amortisation',
                1 => 'amortization',
            ],
        34   =>
            [
                0 => 'amortisations',
                1 => 'amortizations',
            ],
        35   =>
            [
                0 => 'amortise',
                1 => 'amortize',
            ],
        36   =>
            [
                0 => 'amortised',
                1 => 'amortized',
            ],
        37   =>
            [
                0 => 'amortises',
                1 => 'amortizes',
            ],
        38   =>
            [
                0 => 'amortising',
                1 => 'amortizing',
            ],
        39   =>
            [
                0 => 'amphitheatre',
                1 => 'amphitheater',
            ],
        40   =>
            [
                0 => 'amphitheatres',
                1 => 'amphitheaters',
            ],
        41   =>
            [
                0 => 'anaemia',
                1 => 'anemia',
            ],
        42   =>
            [
                0 => 'anaemic',
                1 => 'anemic',
            ],
        43   =>
            [
                0 => 'anaesthesia',
                1 => 'anesthesia',
            ],
        44   =>
            [
                0 => 'anaesthetic',
                1 => 'anesthetic',
            ],
        45   =>
            [
                0 => 'anaesthetics',
                1 => 'anesthetics',
            ],
        46   =>
            [
                0 => 'anaesthetise',
                1 => 'anesthetize',
            ],
        47   =>
            [
                0 => 'anaesthetised',
                1 => 'anesthetized',
            ],
        48   =>
            [
                0 => 'anaesthetises',
                1 => 'anesthetizes',
            ],
        49   =>
            [
                0 => 'anaesthetising',
                1 => 'anesthetizing',
            ],
        50   =>
            [
                0 => 'anaesthetist',
                1 => 'anesthetist',
            ],
        51   =>
            [
                0 => 'anaesthetists',
                1 => 'anesthetists',
            ],
        52   =>
            [
                0 => 'anaesthetize',
                1 => 'anesthetize',
            ],
        53   =>
            [
                0 => 'anaesthetized',
                1 => 'anesthetized',
            ],
        54   =>
            [
                0 => 'anaesthetizes',
                1 => 'anesthetizes',
            ],
        55   =>
            [
                0 => 'anaesthetizing',
                1 => 'anesthetizing',
            ],
        56   =>
            [
                0 => 'analogue',
                1 => 'analog',
            ],
        57   =>
            [
                0 => 'analogues',
                1 => 'analogs',
            ],
        58   =>
            [
                0 => 'analyse',
                1 => 'analyze',
            ],
        59   =>
            [
                0 => 'analysed',
                1 => 'analyzed',
            ],
        60   =>
            [
                0 => 'analyses',
                1 => 'analyzes',
            ],
        61   =>
            [
                0 => 'analysing',
                1 => 'analyzing',
            ],
        62   =>
            [
                0 => 'anglicise',
                1 => 'anglicize',
            ],
        63   =>
            [
                0 => 'anglicised',
                1 => 'anglicized',
            ],
        64   =>
            [
                0 => 'anglicises',
                1 => 'anglicizes',
            ],
        65   =>
            [
                0 => 'anglicising',
                1 => 'anglicizing',
            ],
        66   =>
            [
                0 => 'annualised',
                1 => 'annualized',
            ],
        67   =>
            [
                0 => 'antagonise',
                1 => 'antagonize',
            ],
        68   =>
            [
                0 => 'antagonised',
                1 => 'antagonized',
            ],
        69   =>
            [
                0 => 'antagonises',
                1 => 'antagonizes',
            ],
        70   =>
            [
                0 => 'antagonising',
                1 => 'antagonizing',
            ],
        71   =>
            [
                0 => 'apologise',
                1 => 'apologize',
            ],
        72   =>
            [
                0 => 'apologised',
                1 => 'apologized',
            ],
        73   =>
            [
                0 => 'apologises',
                1 => 'apologizes',
            ],
        74   =>
            [
                0 => 'apologising',
                1 => 'apologizing',
            ],
        75   =>
            [
                0 => 'appal',
                1 => 'appall',
            ],
        76   =>
            [
                0 => 'appals',
                1 => 'appalls',
            ],
        77   =>
            [
                0 => 'appetiser',
                1 => 'appetizer',
            ],
        78   =>
            [
                0 => 'appetisers',
                1 => 'appetizers',
            ],
        79   =>
            [
                0 => 'appetising',
                1 => 'appetizing',
            ],
        80   =>
            [
                0 => 'appetisingly',
                1 => 'appetizingly',
            ],
        81   =>
            [
                0 => 'arbour',
                1 => 'arbor',
            ],
        82   =>
            [
                0 => 'arbours',
                1 => 'arbors',
            ],
        83   =>
            [
                0 => 'archaeological',
                1 => 'archeological',
            ],
        84   =>
            [
                0 => 'archaeologically',
                1 => 'archeologically',
            ],
        85   =>
            [
                0 => 'archaeologist',
                1 => 'archeologist',
            ],
        86   =>
            [
                0 => 'archaeologists',
                1 => 'archeologists',
            ],
        87   =>
            [
                0 => 'archaeology',
                1 => 'archeology',
            ],
        88   =>
            [
                0 => 'ardour',
                1 => 'ardor',
            ],
        89   =>
            [
                0 => 'armour',
                1 => 'armor',
            ],
        90   =>
            [
                0 => 'armoured',
                1 => 'armored',
            ],
        91   =>
            [
                0 => 'armourer',
                1 => 'armorer',
            ],
        92   =>
            [
                0 => 'armourers',
                1 => 'armorers',
            ],
        93   =>
            [
                0 => 'armouries',
                1 => 'armories',
            ],
        94   =>
            [
                0 => 'armoury',
                1 => 'armory',
            ],
        95   =>
            [
                0 => 'artefact',
                1 => 'artifact',
            ],
        96   =>
            [
                0 => 'artefacts',
                1 => 'artifacts',
            ],
        97   =>
            [
                0 => 'authorise',
                1 => 'authorize',
            ],
        98   =>
            [
                0 => 'authorised',
                1 => 'authorized',
            ],
        99   =>
            [
                0 => 'authorises',
                1 => 'authorizes',
            ],
        100  =>
            [
                0 => 'authorising',
                1 => 'authorizing',
            ],
        101  =>
            [
                0 => 'axe',
                1 => 'ax',
            ],
        102  =>
            [
                0 => 'backpedalled',
                1 => 'backpedaled',
            ],
        103  =>
            [
                0 => 'backpedalling',
                1 => 'backpedaling',
            ],
        104  =>
            [
                0 => 'bannister',
                1 => 'banister',
            ],
        105  =>
            [
                0 => 'bannisters',
                1 => 'banisters',
            ],
        106  =>
            [
                0 => 'baptise',
                1 => 'baptize',
            ],
        107  =>
            [
                0 => 'baptised',
                1 => 'baptized',
            ],
        108  =>
            [
                0 => 'baptises',
                1 => 'baptizes',
            ],
        109  =>
            [
                0 => 'baptising',
                1 => 'baptizing',
            ],
        110  =>
            [
                0 => 'bastardise',
                1 => 'bastardize',
            ],
        111  =>
            [
                0 => 'bastardised',
                1 => 'bastardized',
            ],
        112  =>
            [
                0 => 'bastardises',
                1 => 'bastardizes',
            ],
        113  =>
            [
                0 => 'bastardising',
                1 => 'bastardizing',
            ],
        114  =>
            [
                0 => 'battleaxe',
                1 => 'battleax',
            ],
        115  =>
            [
                0 => 'baulk',
                1 => 'balk',
            ],
        116  =>
            [
                0 => 'baulked',
                1 => 'balked',
            ],
        117  =>
            [
                0 => 'baulking',
                1 => 'balking',
            ],
        118  =>
            [
                0 => 'baulks',
                1 => 'balks',
            ],
        119  =>
            [
                0 => 'bedevilled',
                1 => 'bedeviled',
            ],
        120  =>
            [
                0 => 'bedevilling',
                1 => 'bedeviling',
            ],
        121  =>
            [
                0 => 'behaviour',
                1 => 'behavior',
            ],
        122  =>
            [
                0 => 'behavioural',
                1 => 'behavioral',
            ],
        123  =>
            [
                0 => 'behaviourism',
                1 => 'behaviorism',
            ],
        124  =>
            [
                0 => 'behaviourist',
                1 => 'behaviorist',
            ],
        125  =>
            [
                0 => 'behaviourists',
                1 => 'behaviorists',
            ],
        126  =>
            [
                0 => 'behaviours',
                1 => 'behaviors',
            ],
        127  =>
            [
                0 => 'behove',
                1 => 'behoove',
            ],
        128  =>
            [
                0 => 'behoved',
                1 => 'behooved',
            ],
        129  =>
            [
                0 => 'behoves',
                1 => 'behooves',
            ],
        130  =>
            [
                0 => 'bejewelled',
                1 => 'bejeweled',
            ],
        131  =>
            [
                0 => 'belabour',
                1 => 'belabor',
            ],
        132  =>
            [
                0 => 'belaboured',
                1 => 'belabored',
            ],
        133  =>
            [
                0 => 'belabouring',
                1 => 'belaboring',
            ],
        134  =>
            [
                0 => 'belabours',
                1 => 'belabors',
            ],
        135  =>
            [
                0 => 'bevelled',
                1 => 'beveled',
            ],
        136  =>
            [
                0 => 'bevvies',
                1 => 'bevies',
            ],
        137  =>
            [
                0 => 'bevvy',
                1 => 'bevy',
            ],
        138  =>
            [
                0 => 'biassed',
                1 => 'biased',
            ],
        139  =>
            [
                0 => 'biassing',
                1 => 'biasing',
            ],
        140  =>
            [
                0 => 'bingeing',
                1 => 'binging',
            ],
        141  =>
            [
                0 => 'bougainvillaea',
                1 => 'bougainvillea',
            ],
        142  =>
            [
                0 => 'bougainvillaeas',
                1 => 'bougainvilleas',
            ],
        143  =>
            [
                0 => 'bowdlerise',
                1 => 'bowdlerize',
            ],
        144  =>
            [
                0 => 'bowdlerised',
                1 => 'bowdlerized',
            ],
        145  =>
            [
                0 => 'bowdlerises',
                1 => 'bowdlerizes',
            ],
        146  =>
            [
                0 => 'bowdlerising',
                1 => 'bowdlerizing',
            ],
        147  =>
            [
                0 => 'breathalyse',
                1 => 'breathalyze',
            ],
        148  =>
            [
                0 => 'breathalysed',
                1 => 'breathalyzed',
            ],
        149  =>
            [
                0 => 'breathalyser',
                1 => 'breathalyzer',
            ],
        150  =>
            [
                0 => 'breathalysers',
                1 => 'breathalyzers',
            ],
        151  =>
            [
                0 => 'breathalyses',
                1 => 'breathalyzes',
            ],
        152  =>
            [
                0 => 'breathalysing',
                1 => 'breathalyzing',
            ],
        153  =>
            [
                0 => 'brutalise',
                1 => 'brutalize',
            ],
        154  =>
            [
                0 => 'brutalised',
                1 => 'brutalized',
            ],
        155  =>
            [
                0 => 'brutalises',
                1 => 'brutalizes',
            ],
        156  =>
            [
                0 => 'brutalising',
                1 => 'brutalizing',
            ],
        157  =>
            [
                0 => 'buses',
                1 => 'busses',
            ],
        158  =>
            [
                0 => 'busing',
                1 => 'bussing',
            ],
        159  =>
            [
                0 => 'caesarean',
                1 => 'cesarean',
            ],
        160  =>
            [
                0 => 'caesareans',
                1 => 'cesareans',
            ],
        161  =>
            [
                0 => 'calibre',
                1 => 'caliber',
            ],
        162  =>
            [
                0 => 'calibres',
                1 => 'calibers',
            ],
        163  =>
            [
                0 => 'calliper',
                1 => 'caliper',
            ],
        164  =>
            [
                0 => 'callipers',
                1 => 'calipers',
            ],
        165  =>
            [
                0 => 'callisthenics',
                1 => 'calisthenics',
            ],
        166  =>
            [
                0 => 'canalise',
                1 => 'canalize',
            ],
        167  =>
            [
                0 => 'canalised',
                1 => 'canalized',
            ],
        168  =>
            [
                0 => 'canalises',
                1 => 'canalizes',
            ],
        169  =>
            [
                0 => 'canalising',
                1 => 'canalizing',
            ],
        170  =>
            [
                0 => 'cancellation',
                1 => 'cancelation',
            ],
        171  =>
            [
                0 => 'cancellations',
                1 => 'cancelations',
            ],
        172  =>
            [
                0 => 'cancelled',
                1 => 'canceled',
            ],
        173  =>
            [
                0 => 'cancelling',
                1 => 'canceling',
            ],
        174  =>
            [
                0 => 'candour',
                1 => 'candor',
            ],
        175  =>
            [
                0 => 'cannibalise',
                1 => 'cannibalize',
            ],
        176  =>
            [
                0 => 'cannibalised',
                1 => 'cannibalized',
            ],
        177  =>
            [
                0 => 'cannibalises',
                1 => 'cannibalizes',
            ],
        178  =>
            [
                0 => 'cannibalising',
                1 => 'cannibalizing',
            ],
        179  =>
            [
                0 => 'canonise',
                1 => 'canonize',
            ],
        180  =>
            [
                0 => 'canonised',
                1 => 'canonized',
            ],
        181  =>
            [
                0 => 'canonises',
                1 => 'canonizes',
            ],
        182  =>
            [
                0 => 'canonising',
                1 => 'canonizing',
            ],
        183  =>
            [
                0 => 'capitalise',
                1 => 'capitalize',
            ],
        184  =>
            [
                0 => 'capitalised',
                1 => 'capitalized',
            ],
        185  =>
            [
                0 => 'capitalises',
                1 => 'capitalizes',
            ],
        186  =>
            [
                0 => 'capitalising',
                1 => 'capitalizing',
            ],
        187  =>
            [
                0 => 'caramelise',
                1 => 'caramelize',
            ],
        188  =>
            [
                0 => 'caramelised',
                1 => 'caramelized',
            ],
        189  =>
            [
                0 => 'caramelises',
                1 => 'caramelizes',
            ],
        190  =>
            [
                0 => 'caramelising',
                1 => 'caramelizing',
            ],
        191  =>
            [
                0 => 'carbonise',
                1 => 'carbonize',
            ],
        192  =>
            [
                0 => 'carbonised',
                1 => 'carbonized',
            ],
        193  =>
            [
                0 => 'carbonises',
                1 => 'carbonizes',
            ],
        194  =>
            [
                0 => 'carbonising',
                1 => 'carbonizing',
            ],
        195  =>
            [
                0 => 'carolled',
                1 => 'caroled',
            ],
        196  =>
            [
                0 => 'carolling',
                1 => 'caroling',
            ],
        197  =>
            [
                0 => 'catalogue',
                1 => 'catalog',
            ],
        198  =>
            [
                0 => 'catalogued',
                1 => 'cataloged',
            ],
        199  =>
            [
                0 => 'catalogues',
                1 => 'catalogs',
            ],
        200  =>
            [
                0 => 'cataloguing',
                1 => 'cataloging',
            ],
        201  =>
            [
                0 => 'catalyse',
                1 => 'catalyze',
            ],
        202  =>
            [
                0 => 'catalysed',
                1 => 'catalyzed',
            ],
        203  =>
            [
                0 => 'catalyses',
                1 => 'catalyzes',
            ],
        204  =>
            [
                0 => 'catalysing',
                1 => 'catalyzing',
            ],
        205  =>
            [
                0 => 'categorise',
                1 => 'categorize',
            ],
        206  =>
            [
                0 => 'categorised',
                1 => 'categorized',
            ],
        207  =>
            [
                0 => 'categorises',
                1 => 'categorizes',
            ],
        208  =>
            [
                0 => 'categorising',
                1 => 'categorizing',
            ],
        209  =>
            [
                0 => 'cauterise',
                1 => 'cauterize',
            ],
        210  =>
            [
                0 => 'cauterised',
                1 => 'cauterized',
            ],
        211  =>
            [
                0 => 'cauterises',
                1 => 'cauterizes',
            ],
        212  =>
            [
                0 => 'cauterising',
                1 => 'cauterizing',
            ],
        213  =>
            [
                0 => 'cavilled',
                1 => 'caviled',
            ],
        214  =>
            [
                0 => 'cavilling',
                1 => 'caviling',
            ],
        215  =>
            [
                0 => 'centigramme',
                1 => 'centigram',
            ],
        216  =>
            [
                0 => 'centigrammes',
                1 => 'centigrams',
            ],
        217  =>
            [
                0 => 'centilitre',
                1 => 'centiliter',
            ],
        218  =>
            [
                0 => 'centilitres',
                1 => 'centiliters',
            ],
        219  =>
            [
                0 => 'centimetre',
                1 => 'centimeter',
            ],
        220  =>
            [
                0 => 'centimetres',
                1 => 'centimeters',
            ],
        221  =>
            [
                0 => 'centralise',
                1 => 'centralize',
            ],
        222  =>
            [
                0 => 'centralised',
                1 => 'centralized',
            ],
        223  =>
            [
                0 => 'centralises',
                1 => 'centralizes',
            ],
        224  =>
            [
                0 => 'centralising',
                1 => 'centralizing',
            ],
        225  =>
            [
                0 => 'centre',
                1 => 'center',
            ],
        226  =>
            [
                0 => 'centred',
                1 => 'centered',
            ],
        227  =>
            [
                0 => 'centrefold',
                1 => 'centerfold',
            ],
        228  =>
            [
                0 => 'centrefolds',
                1 => 'centerfolds',
            ],
        229  =>
            [
                0 => 'centrepiece',
                1 => 'centerpiece',
            ],
        230  =>
            [
                0 => 'centrepieces',
                1 => 'centerpieces',
            ],
        231  =>
            [
                0 => 'centres',
                1 => 'centers',
            ],
        232  =>
            [
                0 => 'channelled',
                1 => 'channeled',
            ],
        233  =>
            [
                0 => 'channelling',
                1 => 'channeling',
            ],
        234  =>
            [
                0 => 'characterise',
                1 => 'characterize',
            ],
        235  =>
            [
                0 => 'characterised',
                1 => 'characterized',
            ],
        236  =>
            [
                0 => 'characterises',
                1 => 'characterizes',
            ],
        237  =>
            [
                0 => 'characterising',
                1 => 'characterizing',
            ],
        238  =>
            [
                0 => 'cheque',
                1 => 'check',
            ],
        239  =>
            [
                0 => 'chequebook',
                1 => 'checkbook',
            ],
        240  =>
            [
                0 => 'chequebooks',
                1 => 'checkbooks',
            ],
        241  =>
            [
                0 => 'chequered',
                1 => 'checkered',
            ],
        242  =>
            [
                0 => 'cheques',
                1 => 'checks',
            ],
        243  =>
            [
                0 => 'chilli',
                1 => 'chili',
            ],
        244  =>
            [
                0 => 'chimaera',
                1 => 'chimera',
            ],
        245  =>
            [
                0 => 'chimaeras',
                1 => 'chimeras',
            ],
        246  =>
            [
                0 => 'chiselled',
                1 => 'chiseled',
            ],
        247  =>
            [
                0 => 'chiselling',
                1 => 'chiseling',
            ],
        248  =>
            [
                0 => 'circularise',
                1 => 'circularize',
            ],
        249  =>
            [
                0 => 'circularised',
                1 => 'circularized',
            ],
        250  =>
            [
                0 => 'circularises',
                1 => 'circularizes',
            ],
        251  =>
            [
                0 => 'circularising',
                1 => 'circularizing',
            ],
        252  =>
            [
                0 => 'civilise',
                1 => 'civilize',
            ],
        253  =>
            [
                0 => 'civilised',
                1 => 'civilized',
            ],
        254  =>
            [
                0 => 'civilises',
                1 => 'civilizes',
            ],
        255  =>
            [
                0 => 'civilising',
                1 => 'civilizing',
            ],
        256  =>
            [
                0 => 'clamour',
                1 => 'clamor',
            ],
        257  =>
            [
                0 => 'clamoured',
                1 => 'clamored',
            ],
        258  =>
            [
                0 => 'clamouring',
                1 => 'clamoring',
            ],
        259  =>
            [
                0 => 'clamours',
                1 => 'clamors',
            ],
        260  =>
            [
                0 => 'clangour',
                1 => 'clangor',
            ],
        261  =>
            [
                0 => 'clarinettist',
                1 => 'clarinetist',
            ],
        262  =>
            [
                0 => 'clarinettists',
                1 => 'clarinetists',
            ],
        263  =>
            [
                0 => 'collectivise',
                1 => 'collectivize',
            ],
        264  =>
            [
                0 => 'collectivised',
                1 => 'collectivized',
            ],
        265  =>
            [
                0 => 'collectivises',
                1 => 'collectivizes',
            ],
        266  =>
            [
                0 => 'collectivising',
                1 => 'collectivizing',
            ],
        267  =>
            [
                0 => 'colonisation',
                1 => 'colonization',
            ],
        268  =>
            [
                0 => 'colonise',
                1 => 'colonize',
            ],
        269  =>
            [
                0 => 'colonised',
                1 => 'colonized',
            ],
        270  =>
            [
                0 => 'coloniser',
                1 => 'colonizer',
            ],
        271  =>
            [
                0 => 'colonisers',
                1 => 'colonizers',
            ],
        272  =>
            [
                0 => 'colonises',
                1 => 'colonizes',
            ],
        273  =>
            [
                0 => 'colonising',
                1 => 'colonizing',
            ],
        274  =>
            [
                0 => 'colour',
                1 => 'color',
            ],
        275  =>
            [
                0 => 'colourant',
                1 => 'colorant',
            ],
        276  =>
            [
                0 => 'colourants',
                1 => 'colorants',
            ],
        277  =>
            [
                0 => 'coloured',
                1 => 'colored',
            ],
        278  =>
            [
                0 => 'coloureds',
                1 => 'coloreds',
            ],
        279  =>
            [
                0 => 'colourful',
                1 => 'colorful',
            ],
        280  =>
            [
                0 => 'colourfully',
                1 => 'colorfully',
            ],
        281  =>
            [
                0 => 'colouring',
                1 => 'coloring',
            ],
        282  =>
            [
                0 => 'colourize',
                1 => 'colorize',
            ],
        283  =>
            [
                0 => 'colourized',
                1 => 'colorized',
            ],
        284  =>
            [
                0 => 'colourizes',
                1 => 'colorizes',
            ],
        285  =>
            [
                0 => 'colourizing',
                1 => 'colorizing',
            ],
        286  =>
            [
                0 => 'colourless',
                1 => 'colorless',
            ],
        287  =>
            [
                0 => 'colours',
                1 => 'colors',
            ],
        288  =>
            [
                0 => 'commercialise',
                1 => 'commercialize',
            ],
        289  =>
            [
                0 => 'commercialised',
                1 => 'commercialized',
            ],
        290  =>
            [
                0 => 'commercialises',
                1 => 'commercializes',
            ],
        291  =>
            [
                0 => 'commercialising',
                1 => 'commercializing',
            ],
        292  =>
            [
                0 => 'compartmentalise',
                1 => 'compartmentalize',
            ],
        293  =>
            [
                0 => 'compartmentalised',
                1 => 'compartmentalized',
            ],
        294  =>
            [
                0 => 'compartmentalises',
                1 => 'compartmentalizes',
            ],
        295  =>
            [
                0 => 'compartmentalising',
                1 => 'compartmentalizing',
            ],
        296  =>
            [
                0 => 'computerise',
                1 => 'computerize',
            ],
        297  =>
            [
                0 => 'computerised',
                1 => 'computerized',
            ],
        298  =>
            [
                0 => 'computerises',
                1 => 'computerizes',
            ],
        299  =>
            [
                0 => 'computerising',
                1 => 'computerizing',
            ],
        300  =>
            [
                0 => 'conceptualise',
                1 => 'conceptualize',
            ],
        301  =>
            [
                0 => 'conceptualised',
                1 => 'conceptualized',
            ],
        302  =>
            [
                0 => 'conceptualises',
                1 => 'conceptualizes',
            ],
        303  =>
            [
                0 => 'conceptualising',
                1 => 'conceptualizing',
            ],
        304  =>
            [
                0 => 'connexion',
                1 => 'connection',
            ],
        305  =>
            [
                0 => 'connexions',
                1 => 'connections',
            ],
        306  =>
            [
                0 => 'contextualise',
                1 => 'contextualize',
            ],
        307  =>
            [
                0 => 'contextualised',
                1 => 'contextualized',
            ],
        308  =>
            [
                0 => 'contextualises',
                1 => 'contextualizes',
            ],
        309  =>
            [
                0 => 'contextualising',
                1 => 'contextualizing',
            ],
        310  =>
            [
                0 => 'cosier',
                1 => 'cozier',
            ],
        311  =>
            [
                0 => 'cosies',
                1 => 'cozies',
            ],
        312  =>
            [
                0 => 'cosiest',
                1 => 'coziest',
            ],
        313  =>
            [
                0 => 'cosily',
                1 => 'cozily',
            ],
        314  =>
            [
                0 => 'cosiness',
                1 => 'coziness',
            ],
        315  =>
            [
                0 => 'cosy',
                1 => 'cozy',
            ],
        316  =>
            [
                0 => 'councillor',
                1 => 'councilor',
            ],
        317  =>
            [
                0 => 'councillors',
                1 => 'councilors',
            ],
        318  =>
            [
                0 => 'counselled',
                1 => 'counseled',
            ],
        319  =>
            [
                0 => 'counselling',
                1 => 'counseling',
            ],
        320  =>
            [
                0 => 'counsellor',
                1 => 'counselor',
            ],
        321  =>
            [
                0 => 'counsellors',
                1 => 'counselors',
            ],
        322  =>
            [
                0 => 'crenellated',
                1 => 'crenelated',
            ],
        323  =>
            [
                0 => 'criminalise',
                1 => 'criminalize',
            ],
        324  =>
            [
                0 => 'criminalised',
                1 => 'criminalized',
            ],
        325  =>
            [
                0 => 'criminalises',
                1 => 'criminalizes',
            ],
        326  =>
            [
                0 => 'criminalising',
                1 => 'criminalizing',
            ],
        327  =>
            [
                0 => 'criticise',
                1 => 'criticize',
            ],
        328  =>
            [
                0 => 'criticised',
                1 => 'criticized',
            ],
        329  =>
            [
                0 => 'criticises',
                1 => 'criticizes',
            ],
        330  =>
            [
                0 => 'criticising',
                1 => 'criticizing',
            ],
        331  =>
            [
                0 => 'crueller',
                1 => 'crueler',
            ],
        332  =>
            [
                0 => 'cruellest',
                1 => 'cruelest',
            ],
        333  =>
            [
                0 => 'crystallisation',
                1 => 'crystallization',
            ],
        334  =>
            [
                0 => 'crystallise',
                1 => 'crystallize',
            ],
        335  =>
            [
                0 => 'crystallised',
                1 => 'crystallized',
            ],
        336  =>
            [
                0 => 'crystallises',
                1 => 'crystallizes',
            ],
        337  =>
            [
                0 => 'crystallising',
                1 => 'crystallizing',
            ],
        338  =>
            [
                0 => 'cudgelled',
                1 => 'cudgeled',
            ],
        339  =>
            [
                0 => 'cudgelling',
                1 => 'cudgeling',
            ],
        340  =>
            [
                0 => 'customise',
                1 => 'customize',
            ],
        341  =>
            [
                0 => 'customised',
                1 => 'customized',
            ],
        342  =>
            [
                0 => 'customises',
                1 => 'customizes',
            ],
        343  =>
            [
                0 => 'customising',
                1 => 'customizing',
            ],
        344  =>
            [
                0 => 'cypher',
                1 => 'cipher',
            ],
        345  =>
            [
                0 => 'cyphers',
                1 => 'ciphers',
            ],
        346  =>
            [
                0 => 'decentralisation',
                1 => 'decentralization',
            ],
        347  =>
            [
                0 => 'decentralise',
                1 => 'decentralize',
            ],
        348  =>
            [
                0 => 'decentralised',
                1 => 'decentralized',
            ],
        349  =>
            [
                0 => 'decentralises',
                1 => 'decentralizes',
            ],
        350  =>
            [
                0 => 'decentralising',
                1 => 'decentralizing',
            ],
        351  =>
            [
                0 => 'decriminalisation',
                1 => 'decriminalization',
            ],
        352  =>
            [
                0 => 'decriminalise',
                1 => 'decriminalize',
            ],
        353  =>
            [
                0 => 'decriminalised',
                1 => 'decriminalized',
            ],
        354  =>
            [
                0 => 'decriminalises',
                1 => 'decriminalizes',
            ],
        355  =>
            [
                0 => 'decriminalising',
                1 => 'decriminalizing',
            ],
        356  =>
            [
                0 => 'defence',
                1 => 'defense',
            ],
        357  =>
            [
                0 => 'defenceless',
                1 => 'defenseless',
            ],
        358  =>
            [
                0 => 'defences',
                1 => 'defenses',
            ],
        359  =>
            [
                0 => 'dehumanisation',
                1 => 'dehumanization',
            ],
        360  =>
            [
                0 => 'dehumanise',
                1 => 'dehumanize',
            ],
        361  =>
            [
                0 => 'dehumanised',
                1 => 'dehumanized',
            ],
        362  =>
            [
                0 => 'dehumanises',
                1 => 'dehumanizes',
            ],
        363  =>
            [
                0 => 'dehumanising',
                1 => 'dehumanizing',
            ],
        364  =>
            [
                0 => 'demeanour',
                1 => 'demeanor',
            ],
        365  =>
            [
                0 => 'demilitarisation',
                1 => 'demilitarization',
            ],
        366  =>
            [
                0 => 'demilitarise',
                1 => 'demilitarize',
            ],
        367  =>
            [
                0 => 'demilitarised',
                1 => 'demilitarized',
            ],
        368  =>
            [
                0 => 'demilitarises',
                1 => 'demilitarizes',
            ],
        369  =>
            [
                0 => 'demilitarising',
                1 => 'demilitarizing',
            ],
        370  =>
            [
                0 => 'demobilisation',
                1 => 'demobilization',
            ],
        371  =>
            [
                0 => 'demobilise',
                1 => 'demobilize',
            ],
        372  =>
            [
                0 => 'demobilised',
                1 => 'demobilized',
            ],
        373  =>
            [
                0 => 'demobilises',
                1 => 'demobilizes',
            ],
        374  =>
            [
                0 => 'demobilising',
                1 => 'demobilizing',
            ],
        375  =>
            [
                0 => 'democratisation',
                1 => 'democratization',
            ],
        376  =>
            [
                0 => 'democratise',
                1 => 'democratize',
            ],
        377  =>
            [
                0 => 'democratised',
                1 => 'democratized',
            ],
        378  =>
            [
                0 => 'democratises',
                1 => 'democratizes',
            ],
        379  =>
            [
                0 => 'democratising',
                1 => 'democratizing',
            ],
        380  =>
            [
                0 => 'demonise',
                1 => 'demonize',
            ],
        381  =>
            [
                0 => 'demonised',
                1 => 'demonized',
            ],
        382  =>
            [
                0 => 'demonises',
                1 => 'demonizes',
            ],
        383  =>
            [
                0 => 'demonising',
                1 => 'demonizing',
            ],
        384  =>
            [
                0 => 'demoralisation',
                1 => 'demoralization',
            ],
        385  =>
            [
                0 => 'demoralise',
                1 => 'demoralize',
            ],
        386  =>
            [
                0 => 'demoralised',
                1 => 'demoralized',
            ],
        387  =>
            [
                0 => 'demoralises',
                1 => 'demoralizes',
            ],
        388  =>
            [
                0 => 'demoralising',
                1 => 'demoralizing',
            ],
        389  =>
            [
                0 => 'denationalisation',
                1 => 'denationalization',
            ],
        390  =>
            [
                0 => 'denationalise',
                1 => 'denationalize',
            ],
        391  =>
            [
                0 => 'denationalised',
                1 => 'denationalized',
            ],
        392  =>
            [
                0 => 'denationalises',
                1 => 'denationalizes',
            ],
        393  =>
            [
                0 => 'denationalising',
                1 => 'denationalizing',
            ],
        394  =>
            [
                0 => 'deodorise',
                1 => 'deodorize',
            ],
        395  =>
            [
                0 => 'deodorised',
                1 => 'deodorized',
            ],
        396  =>
            [
                0 => 'deodorises',
                1 => 'deodorizes',
            ],
        397  =>
            [
                0 => 'deodorising',
                1 => 'deodorizing',
            ],
        398  =>
            [
                0 => 'depersonalise',
                1 => 'depersonalize',
            ],
        399  =>
            [
                0 => 'depersonalised',
                1 => 'depersonalized',
            ],
        400  =>
            [
                0 => 'depersonalises',
                1 => 'depersonalizes',
            ],
        401  =>
            [
                0 => 'depersonalising',
                1 => 'depersonalizing',
            ],
        402  =>
            [
                0 => 'deputise',
                1 => 'deputize',
            ],
        403  =>
            [
                0 => 'deputised',
                1 => 'deputized',
            ],
        404  =>
            [
                0 => 'deputises',
                1 => 'deputizes',
            ],
        405  =>
            [
                0 => 'deputising',
                1 => 'deputizing',
            ],
        406  =>
            [
                0 => 'desensitisation',
                1 => 'desensitization',
            ],
        407  =>
            [
                0 => 'desensitise',
                1 => 'desensitize',
            ],
        408  =>
            [
                0 => 'desensitised',
                1 => 'desensitized',
            ],
        409  =>
            [
                0 => 'desensitises',
                1 => 'desensitizes',
            ],
        410  =>
            [
                0 => 'desensitising',
                1 => 'desensitizing',
            ],
        411  =>
            [
                0 => 'destabilisation',
                1 => 'destabilization',
            ],
        412  =>
            [
                0 => 'destabilise',
                1 => 'destabilize',
            ],
        413  =>
            [
                0 => 'destabilised',
                1 => 'destabilized',
            ],
        414  =>
            [
                0 => 'destabilises',
                1 => 'destabilizes',
            ],
        415  =>
            [
                0 => 'destabilising',
                1 => 'destabilizing',
            ],
        416  =>
            [
                0 => 'dialled',
                1 => 'dialed',
            ],
        417  =>
            [
                0 => 'dialling',
                1 => 'dialing',
            ],
        418  =>
            [
                0 => 'dialogue',
                1 => 'dialog',
            ],
        419  =>
            [
                0 => 'dialogues',
                1 => 'dialogs',
            ],
        420  =>
            [
                0 => 'diarrhoea',
                1 => 'diarrhea',
            ],
        421  =>
            [
                0 => 'digitise',
                1 => 'digitize',
            ],
        422  =>
            [
                0 => 'digitised',
                1 => 'digitized',
            ],
        423  =>
            [
                0 => 'digitises',
                1 => 'digitizes',
            ],
        424  =>
            [
                0 => 'digitising',
                1 => 'digitizing',
            ],
        425  =>
            [
                0 => 'disc',
                1 => 'disk',
            ],
        426  =>
            [
                0 => 'discolour',
                1 => 'discolor',
            ],
        427  =>
            [
                0 => 'discoloured',
                1 => 'discolored',
            ],
        428  =>
            [
                0 => 'discolouring',
                1 => 'discoloring',
            ],
        429  =>
            [
                0 => 'discolours',
                1 => 'discolors',
            ],
        430  =>
            [
                0 => 'discs',
                1 => 'disks',
            ],
        431  =>
            [
                0 => 'disembowelled',
                1 => 'disemboweled',
            ],
        432  =>
            [
                0 => 'disembowelling',
                1 => 'disemboweling',
            ],
        433  =>
            [
                0 => 'disfavour',
                1 => 'disfavor',
            ],
        434  =>
            [
                0 => 'dishevelled',
                1 => 'disheveled',
            ],
        435  =>
            [
                0 => 'dishonour',
                1 => 'dishonor',
            ],
        436  =>
            [
                0 => 'dishonourable',
                1 => 'dishonorable',
            ],
        437  =>
            [
                0 => 'dishonourably',
                1 => 'dishonorably',
            ],
        438  =>
            [
                0 => 'dishonoured',
                1 => 'dishonored',
            ],
        439  =>
            [
                0 => 'dishonouring',
                1 => 'dishonoring',
            ],
        440  =>
            [
                0 => 'dishonours',
                1 => 'dishonors',
            ],
        441  =>
            [
                0 => 'disorganisation',
                1 => 'disorganization',
            ],
        442  =>
            [
                0 => 'disorganised',
                1 => 'disorganized',
            ],
        443  =>
            [
                0 => 'distil',
                1 => 'distill',
            ],
        444  =>
            [
                0 => 'distils',
                1 => 'distills',
            ],
        445  =>
            [
                0 => 'dramatisation',
                1 => 'dramatization',
            ],
        446  =>
            [
                0 => 'dramatisations',
                1 => 'dramatizations',
            ],
        447  =>
            [
                0 => 'dramatise',
                1 => 'dramatize',
            ],
        448  =>
            [
                0 => 'dramatised',
                1 => 'dramatized',
            ],
        449  =>
            [
                0 => 'dramatises',
                1 => 'dramatizes',
            ],
        450  =>
            [
                0 => 'dramatising',
                1 => 'dramatizing',
            ],
        451  =>
            [
                0 => 'draught',
                1 => 'draft',
            ],
        452  =>
            [
                0 => 'draughtboard',
                1 => 'draftboard',
            ],
        453  =>
            [
                0 => 'draughtboards',
                1 => 'draftboards',
            ],
        454  =>
            [
                0 => 'draughtier',
                1 => 'draftier',
            ],
        455  =>
            [
                0 => 'draughtiest',
                1 => 'draftiest',
            ],
        456  =>
            [
                0 => 'draughts',
                1 => 'drafts',
            ],
        457  =>
            [
                0 => 'draughtsman',
                1 => 'draftsman',
            ],
        458  =>
            [
                0 => 'draughtsmanship',
                1 => 'draftsmanship',
            ],
        459  =>
            [
                0 => 'draughtsmen',
                1 => 'draftsmen',
            ],
        460  =>
            [
                0 => 'draughtswoman',
                1 => 'draftswoman',
            ],
        461  =>
            [
                0 => 'draughtswomen',
                1 => 'draftswomen',
            ],
        462  =>
            [
                0 => 'draughty',
                1 => 'drafty',
            ],
        463  =>
            [
                0 => 'drivelled',
                1 => 'driveled',
            ],
        464  =>
            [
                0 => 'drivelling',
                1 => 'driveling',
            ],
        465  =>
            [
                0 => 'duelled',
                1 => 'dueled',
            ],
        466  =>
            [
                0 => 'duelling',
                1 => 'dueling',
            ],
        467  =>
            [
                0 => 'economise',
                1 => 'economize',
            ],
        468  =>
            [
                0 => 'economised',
                1 => 'economized',
            ],
        469  =>
            [
                0 => 'economises',
                1 => 'economizes',
            ],
        470  =>
            [
                0 => 'economising',
                1 => 'economizing',
            ],
        471  =>
            [
                0 => 'edoema',
                1 => 'edema ',
            ],
        472  =>
            [
                0 => 'editorialise',
                1 => 'editorialize',
            ],
        473  =>
            [
                0 => 'editorialised',
                1 => 'editorialized',
            ],
        474  =>
            [
                0 => 'editorialises',
                1 => 'editorializes',
            ],
        475  =>
            [
                0 => 'editorialising',
                1 => 'editorializing',
            ],
        476  =>
            [
                0 => 'empathise',
                1 => 'empathize',
            ],
        477  =>
            [
                0 => 'empathised',
                1 => 'empathized',
            ],
        478  =>
            [
                0 => 'empathises',
                1 => 'empathizes',
            ],
        479  =>
            [
                0 => 'empathising',
                1 => 'empathizing',
            ],
        480  =>
            [
                0 => 'emphasise',
                1 => 'emphasize',
            ],
        481  =>
            [
                0 => 'emphasised',
                1 => 'emphasized',
            ],
        482  =>
            [
                0 => 'emphasises',
                1 => 'emphasizes',
            ],
        483  =>
            [
                0 => 'emphasising',
                1 => 'emphasizing',
            ],
        484  =>
            [
                0 => 'enamelled',
                1 => 'enameled',
            ],
        485  =>
            [
                0 => 'enamelling',
                1 => 'enameling',
            ],
        486  =>
            [
                0 => 'enamoured',
                1 => 'enamored',
            ],
        487  =>
            [
                0 => 'encyclopaedia',
                1 => 'encyclopedia',
            ],
        488  =>
            [
                0 => 'encyclopaedias',
                1 => 'encyclopedias',
            ],
        489  =>
            [
                0 => 'encyclopaedic',
                1 => 'encyclopedic',
            ],
        490  =>
            [
                0 => 'endeavour',
                1 => 'endeavor',
            ],
        491  =>
            [
                0 => 'endeavoured',
                1 => 'endeavored',
            ],
        492  =>
            [
                0 => 'endeavouring',
                1 => 'endeavoring',
            ],
        493  =>
            [
                0 => 'endeavours',
                1 => 'endeavors',
            ],
        494  =>
            [
                0 => 'energise',
                1 => 'energize',
            ],
        495  =>
            [
                0 => 'energised',
                1 => 'energized',
            ],
        496  =>
            [
                0 => 'energises',
                1 => 'energizes',
            ],
        497  =>
            [
                0 => 'energising',
                1 => 'energizing',
            ],
        498  =>
            [
                0 => 'enrol',
                1 => 'enroll',
            ],
        499  =>
            [
                0 => 'enrols',
                1 => 'enrolls',
            ],
        500  =>
            [
                0 => 'enthral',
                1 => 'enthrall',
            ],
        501  =>
            [
                0 => 'enthrals',
                1 => 'enthralls',
            ],
        502  =>
            [
                0 => 'epaulette',
                1 => 'epaulet',
            ],
        503  =>
            [
                0 => 'epaulettes',
                1 => 'epaulets',
            ],
        504  =>
            [
                0 => 'epicentre',
                1 => 'epicenter',
            ],
        505  =>
            [
                0 => 'epicentres',
                1 => 'epicenters',
            ],
        506  =>
            [
                0 => 'epilogue',
                1 => 'epilog',
            ],
        507  =>
            [
                0 => 'epilogues',
                1 => 'epilogs',
            ],
        508  =>
            [
                0 => 'epitomise',
                1 => 'epitomize',
            ],
        509  =>
            [
                0 => 'epitomised',
                1 => 'epitomized',
            ],
        510  =>
            [
                0 => 'epitomises',
                1 => 'epitomizes',
            ],
        511  =>
            [
                0 => 'epitomising',
                1 => 'epitomizing',
            ],
        512  =>
            [
                0 => 'equalisation',
                1 => 'equalization',
            ],
        513  =>
            [
                0 => 'equalise',
                1 => 'equalize',
            ],
        514  =>
            [
                0 => 'equalised',
                1 => 'equalized',
            ],
        515  =>
            [
                0 => 'equaliser',
                1 => 'equalizer',
            ],
        516  =>
            [
                0 => 'equalisers',
                1 => 'equalizers',
            ],
        517  =>
            [
                0 => 'equalises',
                1 => 'equalizes',
            ],
        518  =>
            [
                0 => 'equalising',
                1 => 'equalizing',
            ],
        519  =>
            [
                0 => 'eulogise',
                1 => 'eulogize',
            ],
        520  =>
            [
                0 => 'eulogised',
                1 => 'eulogized',
            ],
        521  =>
            [
                0 => 'eulogises',
                1 => 'eulogizes',
            ],
        522  =>
            [
                0 => 'eulogising',
                1 => 'eulogizing',
            ],
        523  =>
            [
                0 => 'evangelise',
                1 => 'evangelize',
            ],
        524  =>
            [
                0 => 'evangelised',
                1 => 'evangelized',
            ],
        525  =>
            [
                0 => 'evangelises',
                1 => 'evangelizes',
            ],
        526  =>
            [
                0 => 'evangelising',
                1 => 'evangelizing',
            ],
        527  =>
            [
                0 => 'exorcise',
                1 => 'exorcize',
            ],
        528  =>
            [
                0 => 'exorcised',
                1 => 'exorcized',
            ],
        529  =>
            [
                0 => 'exorcises',
                1 => 'exorcizes',
            ],
        530  =>
            [
                0 => 'exorcising',
                1 => 'exorcizing',
            ],
        531  =>
            [
                0 => 'extemporisation',
                1 => 'extemporization',
            ],
        532  =>
            [
                0 => 'extemporise',
                1 => 'extemporize',
            ],
        533  =>
            [
                0 => 'extemporised',
                1 => 'extemporized',
            ],
        534  =>
            [
                0 => 'extemporises',
                1 => 'extemporizes',
            ],
        535  =>
            [
                0 => 'extemporising',
                1 => 'extemporizing',
            ],
        536  =>
            [
                0 => 'externalisation',
                1 => 'externalization',
            ],
        537  =>
            [
                0 => 'externalisations',
                1 => 'externalizations',
            ],
        538  =>
            [
                0 => 'externalise',
                1 => 'externalize',
            ],
        539  =>
            [
                0 => 'externalised',
                1 => 'externalized',
            ],
        540  =>
            [
                0 => 'externalises',
                1 => 'externalizes',
            ],
        541  =>
            [
                0 => 'externalising',
                1 => 'externalizing',
            ],
        542  =>
            [
                0 => 'factorise',
                1 => 'factorize',
            ],
        543  =>
            [
                0 => 'factorised',
                1 => 'factorized',
            ],
        544  =>
            [
                0 => 'factorises',
                1 => 'factorizes',
            ],
        545  =>
            [
                0 => 'factorising',
                1 => 'factorizing',
            ],
        546  =>
            [
                0 => 'faecal',
                1 => 'fecal',
            ],
        547  =>
            [
                0 => 'faeces',
                1 => 'feces',
            ],
        548  =>
            [
                0 => 'familiarisation',
                1 => 'familiarization',
            ],
        549  =>
            [
                0 => 'familiarise',
                1 => 'familiarize',
            ],
        550  =>
            [
                0 => 'familiarised',
                1 => 'familiarized',
            ],
        551  =>
            [
                0 => 'familiarises',
                1 => 'familiarizes',
            ],
        552  =>
            [
                0 => 'familiarising',
                1 => 'familiarizing',
            ],
        553  =>
            [
                0 => 'fantasise',
                1 => 'fantasize',
            ],
        554  =>
            [
                0 => 'fantasised',
                1 => 'fantasized',
            ],
        555  =>
            [
                0 => 'fantasises',
                1 => 'fantasizes',
            ],
        556  =>
            [
                0 => 'fantasising',
                1 => 'fantasizing',
            ],
        557  =>
            [
                0 => 'favour',
                1 => 'favor',
            ],
        558  =>
            [
                0 => 'favourable',
                1 => 'favorable',
            ],
        559  =>
            [
                0 => 'favourably',
                1 => 'favorably',
            ],
        560  =>
            [
                0 => 'favoured',
                1 => 'favored',
            ],
        561  =>
            [
                0 => 'favouring',
                1 => 'favoring',
            ],
        562  =>
            [
                0 => 'favourite',
                1 => 'favorite',
            ],
        563  =>
            [
                0 => 'favourites',
                1 => 'favorites',
            ],
        564  =>
            [
                0 => 'favouritism',
                1 => 'favoritism',
            ],
        565  =>
            [
                0 => 'favours',
                1 => 'favors',
            ],
        566  =>
            [
                0 => 'feminise',
                1 => 'feminize',
            ],
        567  =>
            [
                0 => 'feminised',
                1 => 'feminized',
            ],
        568  =>
            [
                0 => 'feminises',
                1 => 'feminizes',
            ],
        569  =>
            [
                0 => 'feminising',
                1 => 'feminizing',
            ],
        570  =>
            [
                0 => 'fertilisation',
                1 => 'fertilization',
            ],
        571  =>
            [
                0 => 'fertilise',
                1 => 'fertilize',
            ],
        572  =>
            [
                0 => 'fertilised',
                1 => 'fertilized',
            ],
        573  =>
            [
                0 => 'fertiliser',
                1 => 'fertilizer',
            ],
        574  =>
            [
                0 => 'fertilisers',
                1 => 'fertilizers',
            ],
        575  =>
            [
                0 => 'fertilises',
                1 => 'fertilizes',
            ],
        576  =>
            [
                0 => 'fertilising',
                1 => 'fertilizing',
            ],
        577  =>
            [
                0 => 'fervour',
                1 => 'fervor',
            ],
        578  =>
            [
                0 => 'fibre',
                1 => 'fiber',
            ],
        579  =>
            [
                0 => 'fibreglass',
                1 => 'fiberglass',
            ],
        580  =>
            [
                0 => 'fibres',
                1 => 'fibers',
            ],
        581  =>
            [
                0 => 'fictionalisation',
                1 => 'fictionalization',
            ],
        582  =>
            [
                0 => 'fictionalisations',
                1 => 'fictionalizations',
            ],
        583  =>
            [
                0 => 'fictionalise',
                1 => 'fictionalize',
            ],
        584  =>
            [
                0 => 'fictionalised',
                1 => 'fictionalized',
            ],
        585  =>
            [
                0 => 'fictionalises',
                1 => 'fictionalizes',
            ],
        586  =>
            [
                0 => 'fictionalising',
                1 => 'fictionalizing',
            ],
        587  =>
            [
                0 => 'fillet',
                1 => 'filet',
            ],
        588  =>
            [
                0 => 'filleted ',
                1 => 'fileted ',
            ],
        589  =>
            [
                0 => 'filleting',
                1 => 'fileting',
            ],
        590  =>
            [
                0 => 'fillets ',
                1 => 'filets ',
            ],
        591  =>
            [
                0 => 'finalisation',
                1 => 'finalization',
            ],
        592  =>
            [
                0 => 'finalise',
                1 => 'finalize',
            ],
        593  =>
            [
                0 => 'finalised',
                1 => 'finalized',
            ],
        594  =>
            [
                0 => 'finalises',
                1 => 'finalizes',
            ],
        595  =>
            [
                0 => 'finalising',
                1 => 'finalizing',
            ],
        596  =>
            [
                0 => 'flautist',
                1 => 'flutist',
            ],
        597  =>
            [
                0 => 'flautists',
                1 => 'flutists',
            ],
        598  =>
            [
                0 => 'flavour',
                1 => 'flavor',
            ],
        599  =>
            [
                0 => 'flavoured',
                1 => 'flavored',
            ],
        600  =>
            [
                0 => 'flavouring',
                1 => 'flavoring',
            ],
        601  =>
            [
                0 => 'flavourings',
                1 => 'flavorings',
            ],
        602  =>
            [
                0 => 'flavourless',
                1 => 'flavorless',
            ],
        603  =>
            [
                0 => 'flavours',
                1 => 'flavors',
            ],
        604  =>
            [
                0 => 'flavoursome',
                1 => 'flavorsome',
            ],
        605  =>
            [
                0 => 'flyer / flier ',
                1 => 'flier / flyer ',
            ],
        606  =>
            [
                0 => 'foetal',
                1 => 'fetal',
            ],
        607  =>
            [
                0 => 'foetid',
                1 => 'fetid',
            ],
        608  =>
            [
                0 => 'foetus',
                1 => 'fetus',
            ],
        609  =>
            [
                0 => 'foetuses',
                1 => 'fetuses',
            ],
        610  =>
            [
                0 => 'formalisation',
                1 => 'formalization',
            ],
        611  =>
            [
                0 => 'formalise',
                1 => 'formalize',
            ],
        612  =>
            [
                0 => 'formalised',
                1 => 'formalized',
            ],
        613  =>
            [
                0 => 'formalises',
                1 => 'formalizes',
            ],
        614  =>
            [
                0 => 'formalising',
                1 => 'formalizing',
            ],
        615  =>
            [
                0 => 'fossilisation',
                1 => 'fossilization',
            ],
        616  =>
            [
                0 => 'fossilise',
                1 => 'fossilize',
            ],
        617  =>
            [
                0 => 'fossilised',
                1 => 'fossilized',
            ],
        618  =>
            [
                0 => 'fossilises',
                1 => 'fossilizes',
            ],
        619  =>
            [
                0 => 'fossilising',
                1 => 'fossilizing',
            ],
        620  =>
            [
                0 => 'fraternisation',
                1 => 'fraternization',
            ],
        621  =>
            [
                0 => 'fraternise',
                1 => 'fraternize',
            ],
        622  =>
            [
                0 => 'fraternised',
                1 => 'fraternized',
            ],
        623  =>
            [
                0 => 'fraternises',
                1 => 'fraternizes',
            ],
        624  =>
            [
                0 => 'fraternising',
                1 => 'fraternizing',
            ],
        625  =>
            [
                0 => 'fulfil',
                1 => 'fulfill',
            ],
        626  =>
            [
                0 => 'fulfilment',
                1 => 'fulfillment',
            ],
        627  =>
            [
                0 => 'fulfils',
                1 => 'fulfills',
            ],
        628  =>
            [
                0 => 'funnelled',
                1 => 'funneled',
            ],
        629  =>
            [
                0 => 'funnelling',
                1 => 'funneling',
            ],
        630  =>
            [
                0 => 'galvanise',
                1 => 'galvanize',
            ],
        631  =>
            [
                0 => 'galvanised',
                1 => 'galvanized',
            ],
        632  =>
            [
                0 => 'galvanises',
                1 => 'galvanizes',
            ],
        633  =>
            [
                0 => 'galvanising',
                1 => 'galvanizing',
            ],
        634  =>
            [
                0 => 'gambolled',
                1 => 'gamboled',
            ],
        635  =>
            [
                0 => 'gambolling',
                1 => 'gamboling',
            ],
        636  =>
            [
                0 => 'gaol',
                1 => 'jail',
            ],
        637  =>
            [
                0 => 'gaolbird',
                1 => 'jailbird',
            ],
        638  =>
            [
                0 => 'gaolbirds',
                1 => 'jailbirds',
            ],
        639  =>
            [
                0 => 'gaolbreak',
                1 => 'jailbreak',
            ],
        640  =>
            [
                0 => 'gaolbreaks',
                1 => 'jailbreaks',
            ],
        641  =>
            [
                0 => 'gaoled',
                1 => 'jailed',
            ],
        642  =>
            [
                0 => 'gaoler',
                1 => 'jailer',
            ],
        643  =>
            [
                0 => 'gaolers',
                1 => 'jailers',
            ],
        644  =>
            [
                0 => 'gaoling',
                1 => 'jailing',
            ],
        645  =>
            [
                0 => 'gaols',
                1 => 'jails',
            ],
        646  =>
            [
                0 => 'gases',
                1 => 'gasses',
            ],
        647  =>
            [
                0 => 'gauge',
                1 => 'gage',
            ],
        648  =>
            [
                0 => 'gauged',
                1 => 'gaged',
            ],
        649  =>
            [
                0 => 'gauges',
                1 => 'gages',
            ],
        650  =>
            [
                0 => 'gauging',
                1 => 'gaging',
            ],
        651  =>
            [
                0 => 'generalisation',
                1 => 'generalization',
            ],
        652  =>
            [
                0 => 'generalisations',
                1 => 'generalizations',
            ],
        653  =>
            [
                0 => 'generalise',
                1 => 'generalize',
            ],
        654  =>
            [
                0 => 'generalised',
                1 => 'generalized',
            ],
        655  =>
            [
                0 => 'generalises',
                1 => 'generalizes',
            ],
        656  =>
            [
                0 => 'generalising',
                1 => 'generalizing',
            ],
        657  =>
            [
                0 => 'ghettoise',
                1 => 'ghettoize',
            ],
        658  =>
            [
                0 => 'ghettoised',
                1 => 'ghettoized',
            ],
        659  =>
            [
                0 => 'ghettoises',
                1 => 'ghettoizes',
            ],
        660  =>
            [
                0 => 'ghettoising',
                1 => 'ghettoizing',
            ],
        661  =>
            [
                0 => 'gipsies',
                1 => 'gypsies',
            ],
        662  =>
            [
                0 => 'glamorise',
                1 => 'glamorize',
            ],
        663  =>
            [
                0 => 'glamorised',
                1 => 'glamorized',
            ],
        664  =>
            [
                0 => 'glamorises',
                1 => 'glamorizes',
            ],
        665  =>
            [
                0 => 'glamorising',
                1 => 'glamorizing',
            ],
        666  =>
            [
                0 => 'glamour',
                1 => 'glamor',
            ],
        667  =>
            [
                0 => 'globalisation',
                1 => 'globalization',
            ],
        668  =>
            [
                0 => 'globalise',
                1 => 'globalize',
            ],
        669  =>
            [
                0 => 'globalised',
                1 => 'globalized',
            ],
        670  =>
            [
                0 => 'globalises',
                1 => 'globalizes',
            ],
        671  =>
            [
                0 => 'globalising',
                1 => 'globalizing',
            ],
        672  =>
            [
                0 => 'glueing ',
                1 => 'gluing ',
            ],
        673  =>
            [
                0 => 'goitre',
                1 => 'goiter',
            ],
        674  =>
            [
                0 => 'goitres',
                1 => 'goiters',
            ],
        675  =>
            [
                0 => 'gonorrhoea',
                1 => 'gonorrhea',
            ],
        676  =>
            [
                0 => 'gramme',
                1 => 'gram',
            ],
        677  =>
            [
                0 => 'grammes',
                1 => 'grams',
            ],
        678  =>
            [
                0 => 'gravelled',
                1 => 'graveled',
            ],
        679  =>
            [
                0 => 'grey',
                1 => 'gray',
            ],
        680  =>
            [
                0 => 'greyed',
                1 => 'grayed',
            ],
        681  =>
            [
                0 => 'greying',
                1 => 'graying',
            ],
        682  =>
            [
                0 => 'greyish',
                1 => 'grayish',
            ],
        683  =>
            [
                0 => 'greyness',
                1 => 'grayness',
            ],
        684  =>
            [
                0 => 'greys',
                1 => 'grays',
            ],
        685  =>
            [
                0 => 'grovelled',
                1 => 'groveled',
            ],
        686  =>
            [
                0 => 'grovelling',
                1 => 'groveling',
            ],
        687  =>
            [
                0 => 'groyne',
                1 => 'groin',
            ],
        688  =>
            [
                0 => 'groynes ',
                1 => 'groins',
            ],
        689  =>
            [
                0 => 'gruelling',
                1 => 'grueling',
            ],
        690  =>
            [
                0 => 'gruellingly',
                1 => 'gruelingly',
            ],
        691  =>
            [
                0 => 'gryphon',
                1 => 'griffin',
            ],
        692  =>
            [
                0 => 'gryphons',
                1 => 'griffins',
            ],
        693  =>
            [
                0 => 'gynaecological',
                1 => 'gynecological',
            ],
        694  =>
            [
                0 => 'gynaecologist',
                1 => 'gynecologist',
            ],
        695  =>
            [
                0 => 'gynaecologists',
                1 => 'gynecologists',
            ],
        696  =>
            [
                0 => 'gynaecology',
                1 => 'gynecology',
            ],
        697  =>
            [
                0 => 'haematological',
                1 => 'hematological',
            ],
        698  =>
            [
                0 => 'haematologist',
                1 => 'hematologist',
            ],
        699  =>
            [
                0 => 'haematologists',
                1 => 'hematologists',
            ],
        700  =>
            [
                0 => 'haematology',
                1 => 'hematology',
            ],
        701  =>
            [
                0 => 'haemoglobin',
                1 => 'hemoglobin',
            ],
        702  =>
            [
                0 => 'haemophilia',
                1 => 'hemophilia',
            ],
        703  =>
            [
                0 => 'haemophiliac',
                1 => 'hemophiliac',
            ],
        704  =>
            [
                0 => 'haemophiliacs',
                1 => 'hemophiliacs',
            ],
        705  =>
            [
                0 => 'haemorrhage',
                1 => 'hemorrhage',
            ],
        706  =>
            [
                0 => 'haemorrhaged',
                1 => 'hemorrhaged',
            ],
        707  =>
            [
                0 => 'haemorrhages',
                1 => 'hemorrhages',
            ],
        708  =>
            [
                0 => 'haemorrhaging',
                1 => 'hemorrhaging',
            ],
        709  =>
            [
                0 => 'haemorrhoids',
                1 => 'hemorrhoids',
            ],
        710  =>
            [
                0 => 'harbour',
                1 => 'harbor',
            ],
        711  =>
            [
                0 => 'harboured',
                1 => 'harbored',
            ],
        712  =>
            [
                0 => 'harbouring',
                1 => 'harboring',
            ],
        713  =>
            [
                0 => 'harbours',
                1 => 'harbors',
            ],
        714  =>
            [
                0 => 'harmonisation',
                1 => 'harmonization',
            ],
        715  =>
            [
                0 => 'harmonise',
                1 => 'harmonize',
            ],
        716  =>
            [
                0 => 'harmonised',
                1 => 'harmonized',
            ],
        717  =>
            [
                0 => 'harmonises',
                1 => 'harmonizes',
            ],
        718  =>
            [
                0 => 'harmonising',
                1 => 'harmonizing',
            ],
        719  =>
            [
                0 => 'homoeopath',
                1 => 'homeopath',
            ],
        720  =>
            [
                0 => 'homoeopathic',
                1 => 'homeopathic',
            ],
        721  =>
            [
                0 => 'homoeopaths',
                1 => 'homeopaths',
            ],
        722  =>
            [
                0 => 'homoeopathy',
                1 => 'homeopathy',
            ],
        723  =>
            [
                0 => 'homogenise',
                1 => 'homogenize',
            ],
        724  =>
            [
                0 => 'homogenised',
                1 => 'homogenized',
            ],
        725  =>
            [
                0 => 'homogenises',
                1 => 'homogenizes',
            ],
        726  =>
            [
                0 => 'homogenising',
                1 => 'homogenizing',
            ],
        727  =>
            [
                0 => 'honour',
                1 => 'honor',
            ],
        728  =>
            [
                0 => 'honourable',
                1 => 'honorable',
            ],
        729  =>
            [
                0 => 'honourably',
                1 => 'honorably',
            ],
        730  =>
            [
                0 => 'honoured',
                1 => 'honored',
            ],
        731  =>
            [
                0 => 'honouring',
                1 => 'honoring',
            ],
        732  =>
            [
                0 => 'honours',
                1 => 'honors',
            ],
        733  =>
            [
                0 => 'hospitalisation',
                1 => 'hospitalization',
            ],
        734  =>
            [
                0 => 'hospitalise',
                1 => 'hospitalize',
            ],
        735  =>
            [
                0 => 'hospitalised',
                1 => 'hospitalized',
            ],
        736  =>
            [
                0 => 'hospitalises',
                1 => 'hospitalizes',
            ],
        737  =>
            [
                0 => 'hospitalising',
                1 => 'hospitalizing',
            ],
        738  =>
            [
                0 => 'humanise',
                1 => 'humanize',
            ],
        739  =>
            [
                0 => 'humanised',
                1 => 'humanized',
            ],
        740  =>
            [
                0 => 'humanises',
                1 => 'humanizes',
            ],
        741  =>
            [
                0 => 'humanising',
                1 => 'humanizing',
            ],
        742  =>
            [
                0 => 'humour',
                1 => 'humor',
            ],
        743  =>
            [
                0 => 'humoured',
                1 => 'humored',
            ],
        744  =>
            [
                0 => 'humouring',
                1 => 'humoring',
            ],
        745  =>
            [
                0 => 'humourless',
                1 => 'humorless',
            ],
        746  =>
            [
                0 => 'humours',
                1 => 'humors',
            ],
        747  =>
            [
                0 => 'hybridise',
                1 => 'hybridize',
            ],
        748  =>
            [
                0 => 'hybridised',
                1 => 'hybridized',
            ],
        749  =>
            [
                0 => 'hybridises',
                1 => 'hybridizes',
            ],
        750  =>
            [
                0 => 'hybridising',
                1 => 'hybridizing',
            ],
        751  =>
            [
                0 => 'hypnotise',
                1 => 'hypnotize',
            ],
        752  =>
            [
                0 => 'hypnotised',
                1 => 'hypnotized',
            ],
        753  =>
            [
                0 => 'hypnotises',
                1 => 'hypnotizes',
            ],
        754  =>
            [
                0 => 'hypnotising',
                1 => 'hypnotizing',
            ],
        755  =>
            [
                0 => 'hypothesise',
                1 => 'hypothesize',
            ],
        756  =>
            [
                0 => 'hypothesised',
                1 => 'hypothesized',
            ],
        757  =>
            [
                0 => 'hypothesises',
                1 => 'hypothesizes',
            ],
        758  =>
            [
                0 => 'hypothesising',
                1 => 'hypothesizing',
            ],
        759  =>
            [
                0 => 'idealisation',
                1 => 'idealization',
            ],
        760  =>
            [
                0 => 'idealise',
                1 => 'idealize',
            ],
        761  =>
            [
                0 => 'idealised',
                1 => 'idealized',
            ],
        762  =>
            [
                0 => 'idealises',
                1 => 'idealizes',
            ],
        763  =>
            [
                0 => 'idealising',
                1 => 'idealizing',
            ],
        764  =>
            [
                0 => 'idolise',
                1 => 'idolize',
            ],
        765  =>
            [
                0 => 'idolised',
                1 => 'idolized',
            ],
        766  =>
            [
                0 => 'idolises',
                1 => 'idolizes',
            ],
        767  =>
            [
                0 => 'idolising',
                1 => 'idolizing',
            ],
        768  =>
            [
                0 => 'immobilisation',
                1 => 'immobilization',
            ],
        769  =>
            [
                0 => 'immobilise',
                1 => 'immobilize',
            ],
        770  =>
            [
                0 => 'immobilised',
                1 => 'immobilized',
            ],
        771  =>
            [
                0 => 'immobiliser',
                1 => 'immobilizer',
            ],
        772  =>
            [
                0 => 'immobilisers',
                1 => 'immobilizers',
            ],
        773  =>
            [
                0 => 'immobilises',
                1 => 'immobilizes',
            ],
        774  =>
            [
                0 => 'immobilising',
                1 => 'immobilizing',
            ],
        775  =>
            [
                0 => 'immortalise',
                1 => 'immortalize',
            ],
        776  =>
            [
                0 => 'immortalised',
                1 => 'immortalized',
            ],
        777  =>
            [
                0 => 'immortalises',
                1 => 'immortalizes',
            ],
        778  =>
            [
                0 => 'immortalising',
                1 => 'immortalizing',
            ],
        779  =>
            [
                0 => 'immunisation',
                1 => 'immunization',
            ],
        780  =>
            [
                0 => 'immunise',
                1 => 'immunize',
            ],
        781  =>
            [
                0 => 'immunised',
                1 => 'immunized',
            ],
        782  =>
            [
                0 => 'immunises',
                1 => 'immunizes',
            ],
        783  =>
            [
                0 => 'immunising',
                1 => 'immunizing',
            ],
        784  =>
            [
                0 => 'impanelled',
                1 => 'impaneled',
            ],
        785  =>
            [
                0 => 'impanelling',
                1 => 'impaneling',
            ],
        786  =>
            [
                0 => 'imperilled',
                1 => 'imperiled',
            ],
        787  =>
            [
                0 => 'imperilling',
                1 => 'imperiling',
            ],
        788  =>
            [
                0 => 'individualise',
                1 => 'individualize',
            ],
        789  =>
            [
                0 => 'individualised',
                1 => 'individualized',
            ],
        790  =>
            [
                0 => 'individualises',
                1 => 'individualizes',
            ],
        791  =>
            [
                0 => 'individualising',
                1 => 'individualizing',
            ],
        792  =>
            [
                0 => 'industrialise',
                1 => 'industrialize',
            ],
        793  =>
            [
                0 => 'industrialised',
                1 => 'industrialized',
            ],
        794  =>
            [
                0 => 'industrialises',
                1 => 'industrializes',
            ],
        795  =>
            [
                0 => 'industrialising',
                1 => 'industrializing',
            ],
        796  =>
            [
                0 => 'inflexion',
                1 => 'inflection',
            ],
        797  =>
            [
                0 => 'inflexions',
                1 => 'inflections',
            ],
        798  =>
            [
                0 => 'initialise',
                1 => 'initialize',
            ],
        799  =>
            [
                0 => 'initialised',
                1 => 'initialized',
            ],
        800  =>
            [
                0 => 'initialises',
                1 => 'initializes',
            ],
        801  =>
            [
                0 => 'initialising',
                1 => 'initializing',
            ],
        802  =>
            [
                0 => 'initialled',
                1 => 'initialed',
            ],
        803  =>
            [
                0 => 'initialling',
                1 => 'initialing',
            ],
        804  =>
            [
                0 => 'instal',
                1 => 'install',
            ],
        805  =>
            [
                0 => 'instalment',
                1 => 'installment',
            ],
        806  =>
            [
                0 => 'instalments',
                1 => 'installments',
            ],
        807  =>
            [
                0 => 'instals',
                1 => 'installs',
            ],
        808  =>
            [
                0 => 'instil',
                1 => 'instill',
            ],
        809  =>
            [
                0 => 'instils',
                1 => 'instills',
            ],
        810  =>
            [
                0 => 'institutionalisation',
                1 => 'institutionalization',
            ],
        811  =>
            [
                0 => 'institutionalise',
                1 => 'institutionalize',
            ],
        812  =>
            [
                0 => 'institutionalised',
                1 => 'institutionalized',
            ],
        813  =>
            [
                0 => 'institutionalises',
                1 => 'institutionalizes',
            ],
        814  =>
            [
                0 => 'institutionalising',
                1 => 'institutionalizing',
            ],
        815  =>
            [
                0 => 'intellectualise',
                1 => 'intellectualize',
            ],
        816  =>
            [
                0 => 'intellectualised',
                1 => 'intellectualized',
            ],
        817  =>
            [
                0 => 'intellectualises',
                1 => 'intellectualizes',
            ],
        818  =>
            [
                0 => 'intellectualising',
                1 => 'intellectualizing',
            ],
        819  =>
            [
                0 => 'internalisation',
                1 => 'internalization',
            ],
        820  =>
            [
                0 => 'internalise',
                1 => 'internalize',
            ],
        821  =>
            [
                0 => 'internalised',
                1 => 'internalized',
            ],
        822  =>
            [
                0 => 'internalises',
                1 => 'internalizes',
            ],
        823  =>
            [
                0 => 'internalising',
                1 => 'internalizing',
            ],
        824  =>
            [
                0 => 'internationalisation',
                1 => 'internationalization',
            ],
        825  =>
            [
                0 => 'internationalise',
                1 => 'internationalize',
            ],
        826  =>
            [
                0 => 'internationalised',
                1 => 'internationalized',
            ],
        827  =>
            [
                0 => 'internationalises',
                1 => 'internationalizes',
            ],
        828  =>
            [
                0 => 'internationalising',
                1 => 'internationalizing',
            ],
        829  =>
            [
                0 => 'ionisation',
                1 => 'ionization',
            ],
        830  =>
            [
                0 => 'ionise',
                1 => 'ionize',
            ],
        831  =>
            [
                0 => 'ionised',
                1 => 'ionized',
            ],
        832  =>
            [
                0 => 'ioniser',
                1 => 'ionizer',
            ],
        833  =>
            [
                0 => 'ionisers',
                1 => 'ionizers',
            ],
        834  =>
            [
                0 => 'ionises',
                1 => 'ionizes',
            ],
        835  =>
            [
                0 => 'ionising',
                1 => 'ionizing',
            ],
        836  =>
            [
                0 => 'italicise',
                1 => 'italicize',
            ],
        837  =>
            [
                0 => 'italicised',
                1 => 'italicized',
            ],
        838  =>
            [
                0 => 'italicises',
                1 => 'italicizes',
            ],
        839  =>
            [
                0 => 'italicising',
                1 => 'italicizing',
            ],
        840  =>
            [
                0 => 'itemise',
                1 => 'itemize',
            ],
        841  =>
            [
                0 => 'itemised',
                1 => 'itemized',
            ],
        842  =>
            [
                0 => 'itemises',
                1 => 'itemizes',
            ],
        843  =>
            [
                0 => 'itemising',
                1 => 'itemizing',
            ],
        844  =>
            [
                0 => 'jeopardise',
                1 => 'jeopardize',
            ],
        845  =>
            [
                0 => 'jeopardised',
                1 => 'jeopardized',
            ],
        846  =>
            [
                0 => 'jeopardises',
                1 => 'jeopardizes',
            ],
        847  =>
            [
                0 => 'jeopardising',
                1 => 'jeopardizing',
            ],
        848  =>
            [
                0 => 'jewelled',
                1 => 'jeweled',
            ],
        849  =>
            [
                0 => 'jeweller',
                1 => 'jeweler',
            ],
        850  =>
            [
                0 => 'jewellers',
                1 => 'jewelers',
            ],
        851  =>
            [
                0 => 'jewellery',
                1 => 'jewelry',
            ],
        852  =>
            [
                0 => 'judgement ',
                1 => 'judgment',
            ],
        853  =>
            [
                0 => 'kilogramme',
                1 => 'kilogram',
            ],
        854  =>
            [
                0 => 'kilogrammes',
                1 => 'kilograms',
            ],
        855  =>
            [
                0 => 'kilometre',
                1 => 'kilometer',
            ],
        856  =>
            [
                0 => 'kilometres',
                1 => 'kilometers',
            ],
        857  =>
            [
                0 => 'labelled',
                1 => 'labeled',
            ],
        858  =>
            [
                0 => 'labelling',
                1 => 'labeling',
            ],
        859  =>
            [
                0 => 'labour',
                1 => 'labor',
            ],
        860  =>
            [
                0 => 'laboured',
                1 => 'labored',
            ],
        861  =>
            [
                0 => 'labourer',
                1 => 'laborer',
            ],
        862  =>
            [
                0 => 'labourers',
                1 => 'laborers',
            ],
        863  =>
            [
                0 => 'labouring',
                1 => 'laboring',
            ],
        864  =>
            [
                0 => 'labours',
                1 => 'labors',
            ],
        865  =>
            [
                0 => 'lacklustre',
                1 => 'lackluster',
            ],
        866  =>
            [
                0 => 'legalisation',
                1 => 'legalization',
            ],
        867  =>
            [
                0 => 'legalise',
                1 => 'legalize',
            ],
        868  =>
            [
                0 => 'legalised',
                1 => 'legalized',
            ],
        869  =>
            [
                0 => 'legalises',
                1 => 'legalizes',
            ],
        870  =>
            [
                0 => 'legalising',
                1 => 'legalizing',
            ],
        871  =>
            [
                0 => 'legitimise',
                1 => 'legitimize',
            ],
        872  =>
            [
                0 => 'legitimised',
                1 => 'legitimized',
            ],
        873  =>
            [
                0 => 'legitimises',
                1 => 'legitimizes',
            ],
        874  =>
            [
                0 => 'legitimising',
                1 => 'legitimizing',
            ],
        875  =>
            [
                0 => 'leukaemia',
                1 => 'leukemia',
            ],
        876  =>
            [
                0 => 'levelled',
                1 => 'leveled',
            ],
        877  =>
            [
                0 => 'leveller',
                1 => 'leveler',
            ],
        878  =>
            [
                0 => 'levellers',
                1 => 'levelers',
            ],
        879  =>
            [
                0 => 'levelling',
                1 => 'leveling',
            ],
        880  =>
            [
                0 => 'libelled',
                1 => 'libeled',
            ],
        881  =>
            [
                0 => 'libelling',
                1 => 'libeling',
            ],
        882  =>
            [
                0 => 'libellous',
                1 => 'libelous',
            ],
        883  =>
            [
                0 => 'liberalisation',
                1 => 'liberalization',
            ],
        884  =>
            [
                0 => 'liberalise',
                1 => 'liberalize',
            ],
        885  =>
            [
                0 => 'liberalised',
                1 => 'liberalized',
            ],
        886  =>
            [
                0 => 'liberalises',
                1 => 'liberalizes',
            ],
        887  =>
            [
                0 => 'liberalising',
                1 => 'liberalizing',
            ],
        888  =>
            [
                0 => 'licence',
                1 => 'license',
            ],
        889  =>
            [
                0 => 'licenced',
                1 => 'licensed',
            ],
        890  =>
            [
                0 => 'licences',
                1 => 'licenses',
            ],
        891  =>
            [
                0 => 'licencing',
                1 => 'licensing',
            ],
        892  =>
            [
                0 => 'likeable',
                1 => 'likable ',
            ],
        893  =>
            [
                0 => 'lionisation',
                1 => 'lionization',
            ],
        894  =>
            [
                0 => 'lionise',
                1 => 'lionize',
            ],
        895  =>
            [
                0 => 'lionised',
                1 => 'lionized',
            ],
        896  =>
            [
                0 => 'lionises',
                1 => 'lionizes',
            ],
        897  =>
            [
                0 => 'lionising',
                1 => 'lionizing',
            ],
        898  =>
            [
                0 => 'liquidise',
                1 => 'liquidize',
            ],
        899  =>
            [
                0 => 'liquidised',
                1 => 'liquidized',
            ],
        900  =>
            [
                0 => 'liquidiser',
                1 => 'liquidizer',
            ],
        901  =>
            [
                0 => 'liquidisers',
                1 => 'liquidizers',
            ],
        902  =>
            [
                0 => 'liquidises',
                1 => 'liquidizes',
            ],
        903  =>
            [
                0 => 'liquidising',
                1 => 'liquidizing',
            ],
        904  =>
            [
                0 => 'litre',
                1 => 'liter',
            ],
        905  =>
            [
                0 => 'litres',
                1 => 'liters',
            ],
        906  =>
            [
                0 => 'localise',
                1 => 'localize',
            ],
        907  =>
            [
                0 => 'localised',
                1 => 'localized',
            ],
        908  =>
            [
                0 => 'localises',
                1 => 'localizes',
            ],
        909  =>
            [
                0 => 'localising',
                1 => 'localizing',
            ],
        910  =>
            [
                0 => 'louvre',
                1 => 'louver',
            ],
        911  =>
            [
                0 => 'louvred',
                1 => 'louvered',
            ],
        912  =>
            [
                0 => 'louvres',
                1 => 'louvers ',
            ],
        913  =>
            [
                0 => 'lustre',
                1 => 'luster',
            ],
        914  =>
            [
                0 => 'magnetise',
                1 => 'magnetize',
            ],
        915  =>
            [
                0 => 'magnetised',
                1 => 'magnetized',
            ],
        916  =>
            [
                0 => 'magnetises',
                1 => 'magnetizes',
            ],
        917  =>
            [
                0 => 'magnetising',
                1 => 'magnetizing',
            ],
        918  =>
            [
                0 => 'manoeuvrability',
                1 => 'maneuverability',
            ],
        919  =>
            [
                0 => 'manoeuvrable',
                1 => 'maneuverable',
            ],
        920  =>
            [
                0 => 'manoeuvre',
                1 => 'maneuver',
            ],
        921  =>
            [
                0 => 'manoeuvred',
                1 => 'maneuvered',
            ],
        922  =>
            [
                0 => 'manoeuvres',
                1 => 'maneuvers',
            ],
        923  =>
            [
                0 => 'manoeuvring',
                1 => 'maneuvering',
            ],
        924  =>
            [
                0 => 'manoeuvrings',
                1 => 'maneuverings',
            ],
        925  =>
            [
                0 => 'marginalisation',
                1 => 'marginalization',
            ],
        926  =>
            [
                0 => 'marginalise',
                1 => 'marginalize',
            ],
        927  =>
            [
                0 => 'marginalised',
                1 => 'marginalized',
            ],
        928  =>
            [
                0 => 'marginalises',
                1 => 'marginalizes',
            ],
        929  =>
            [
                0 => 'marginalising',
                1 => 'marginalizing',
            ],
        930  =>
            [
                0 => 'marshalled',
                1 => 'marshaled',
            ],
        931  =>
            [
                0 => 'marshalling',
                1 => 'marshaling',
            ],
        932  =>
            [
                0 => 'marvelled',
                1 => 'marveled',
            ],
        933  =>
            [
                0 => 'marvelling',
                1 => 'marveling',
            ],
        934  =>
            [
                0 => 'marvellous',
                1 => 'marvelous',
            ],
        935  =>
            [
                0 => 'marvellously',
                1 => 'marvelously',
            ],
        936  =>
            [
                0 => 'materialisation',
                1 => 'materialization',
            ],
        937  =>
            [
                0 => 'materialise',
                1 => 'materialize',
            ],
        938  =>
            [
                0 => 'materialised',
                1 => 'materialized',
            ],
        939  =>
            [
                0 => 'materialises',
                1 => 'materializes',
            ],
        940  =>
            [
                0 => 'materialising',
                1 => 'materializing',
            ],
        941  =>
            [
                0 => 'maximisation',
                1 => 'maximization',
            ],
        942  =>
            [
                0 => 'maximise',
                1 => 'maximize',
            ],
        943  =>
            [
                0 => 'maximised',
                1 => 'maximized',
            ],
        944  =>
            [
                0 => 'maximises',
                1 => 'maximizes',
            ],
        945  =>
            [
                0 => 'maximising',
                1 => 'maximizing',
            ],
        946  =>
            [
                0 => 'meagre',
                1 => 'meager',
            ],
        947  =>
            [
                0 => 'mechanisation',
                1 => 'mechanization',
            ],
        948  =>
            [
                0 => 'mechanise',
                1 => 'mechanize',
            ],
        949  =>
            [
                0 => 'mechanised',
                1 => 'mechanized',
            ],
        950  =>
            [
                0 => 'mechanises',
                1 => 'mechanizes',
            ],
        951  =>
            [
                0 => 'mechanising',
                1 => 'mechanizing',
            ],
        952  =>
            [
                0 => 'mediaeval',
                1 => 'medieval',
            ],
        953  =>
            [
                0 => 'memorialise',
                1 => 'memorialize',
            ],
        954  =>
            [
                0 => 'memorialised',
                1 => 'memorialized',
            ],
        955  =>
            [
                0 => 'memorialises',
                1 => 'memorializes',
            ],
        956  =>
            [
                0 => 'memorialising',
                1 => 'memorializing',
            ],
        957  =>
            [
                0 => 'memorise',
                1 => 'memorize',
            ],
        958  =>
            [
                0 => 'memorised',
                1 => 'memorized',
            ],
        959  =>
            [
                0 => 'memorises',
                1 => 'memorizes',
            ],
        960  =>
            [
                0 => 'memorising',
                1 => 'memorizing',
            ],
        961  =>
            [
                0 => 'mesmerise',
                1 => 'mesmerize',
            ],
        962  =>
            [
                0 => 'mesmerised',
                1 => 'mesmerized',
            ],
        963  =>
            [
                0 => 'mesmerises',
                1 => 'mesmerizes',
            ],
        964  =>
            [
                0 => 'mesmerising',
                1 => 'mesmerizing',
            ],
        965  =>
            [
                0 => 'metabolise',
                1 => 'metabolize',
            ],
        966  =>
            [
                0 => 'metabolised',
                1 => 'metabolized',
            ],
        967  =>
            [
                0 => 'metabolises',
                1 => 'metabolizes',
            ],
        968  =>
            [
                0 => 'metabolising',
                1 => 'metabolizing',
            ],
        969  =>
            [
                0 => 'metre',
                1 => 'meter',
            ],
        970  =>
            [
                0 => 'metres',
                1 => 'meters',
            ],
        971  =>
            [
                0 => 'micrometre',
                1 => 'micrometer',
            ],
        972  =>
            [
                0 => 'micrometres',
                1 => 'micrometers',
            ],
        973  =>
            [
                0 => 'militarise',
                1 => 'militarize',
            ],
        974  =>
            [
                0 => 'militarised',
                1 => 'militarized',
            ],
        975  =>
            [
                0 => 'militarises',
                1 => 'militarizes',
            ],
        976  =>
            [
                0 => 'militarising',
                1 => 'militarizing',
            ],
        977  =>
            [
                0 => 'milligramme',
                1 => 'milligram',
            ],
        978  =>
            [
                0 => 'milligrammes',
                1 => 'milligrams',
            ],
        979  =>
            [
                0 => 'millilitre',
                1 => 'milliliter',
            ],
        980  =>
            [
                0 => 'millilitres',
                1 => 'milliliters',
            ],
        981  =>
            [
                0 => 'millimetre',
                1 => 'millimeter',
            ],
        982  =>
            [
                0 => 'millimetres',
                1 => 'millimeters',
            ],
        983  =>
            [
                0 => 'miniaturisation',
                1 => 'miniaturization',
            ],
        984  =>
            [
                0 => 'miniaturise',
                1 => 'miniaturize',
            ],
        985  =>
            [
                0 => 'miniaturised',
                1 => 'miniaturized',
            ],
        986  =>
            [
                0 => 'miniaturises',
                1 => 'miniaturizes',
            ],
        987  =>
            [
                0 => 'miniaturising',
                1 => 'miniaturizing',
            ],
        988  =>
            [
                0 => 'minibuses',
                1 => 'minibusses ',
            ],
        989  =>
            [
                0 => 'minimise',
                1 => 'minimize',
            ],
        990  =>
            [
                0 => 'minimised',
                1 => 'minimized',
            ],
        991  =>
            [
                0 => 'minimises',
                1 => 'minimizes',
            ],
        992  =>
            [
                0 => 'minimising',
                1 => 'minimizing',
            ],
        993  =>
            [
                0 => 'misbehaviour',
                1 => 'misbehavior',
            ],
        994  =>
            [
                0 => 'misdemeanour',
                1 => 'misdemeanor',
            ],
        995  =>
            [
                0 => 'misdemeanours',
                1 => 'misdemeanors',
            ],
        996  =>
            [
                0 => 'misspelt',
                1 => 'misspelled ',
            ],
        997  =>
            [
                0 => 'mitre',
                1 => 'miter',
            ],
        998  =>
            [
                0 => 'mitres',
                1 => 'miters',
            ],
        999  =>
            [
                0 => 'mobilisation',
                1 => 'mobilization',
            ],
        1000 =>
            [
                0 => 'mobilise',
                1 => 'mobilize',
            ],
        1001 =>
            [
                0 => 'mobilised',
                1 => 'mobilized',
            ],
        1002 =>
            [
                0 => 'mobilises',
                1 => 'mobilizes',
            ],
        1003 =>
            [
                0 => 'mobilising',
                1 => 'mobilizing',
            ],
        1004 =>
            [
                0 => 'modelled',
                1 => 'modeled',
            ],
        1005 =>
            [
                0 => 'modeller',
                1 => 'modeler',
            ],
        1006 =>
            [
                0 => 'modellers',
                1 => 'modelers',
            ],
        1007 =>
            [
                0 => 'modelling',
                1 => 'modeling',
            ],
        1008 =>
            [
                0 => 'modernise',
                1 => 'modernize',
            ],
        1009 =>
            [
                0 => 'modernised',
                1 => 'modernized',
            ],
        1010 =>
            [
                0 => 'modernises',
                1 => 'modernizes',
            ],
        1011 =>
            [
                0 => 'modernising',
                1 => 'modernizing',
            ],
        1012 =>
            [
                0 => 'moisturise',
                1 => 'moisturize',
            ],
        1013 =>
            [
                0 => 'moisturised',
                1 => 'moisturized',
            ],
        1014 =>
            [
                0 => 'moisturiser',
                1 => 'moisturizer',
            ],
        1015 =>
            [
                0 => 'moisturisers',
                1 => 'moisturizers',
            ],
        1016 =>
            [
                0 => 'moisturises',
                1 => 'moisturizes',
            ],
        1017 =>
            [
                0 => 'moisturising',
                1 => 'moisturizing',
            ],
        1018 =>
            [
                0 => 'monologue',
                1 => 'monolog',
            ],
        1019 =>
            [
                0 => 'monologues',
                1 => 'monologs',
            ],
        1020 =>
            [
                0 => 'monopolisation',
                1 => 'monopolization',
            ],
        1021 =>
            [
                0 => 'monopolise',
                1 => 'monopolize',
            ],
        1022 =>
            [
                0 => 'monopolised',
                1 => 'monopolized',
            ],
        1023 =>
            [
                0 => 'monopolises',
                1 => 'monopolizes',
            ],
        1024 =>
            [
                0 => 'monopolising',
                1 => 'monopolizing',
            ],
        1025 =>
            [
                0 => 'moralise',
                1 => 'moralize',
            ],
        1026 =>
            [
                0 => 'moralised',
                1 => 'moralized',
            ],
        1027 =>
            [
                0 => 'moralises',
                1 => 'moralizes',
            ],
        1028 =>
            [
                0 => 'moralising',
                1 => 'moralizing',
            ],
        1029 =>
            [
                0 => 'motorised',
                1 => 'motorized',
            ],
        1030 =>
            [
                0 => 'mould',
                1 => 'mold',
            ],
        1031 =>
            [
                0 => 'moulded',
                1 => 'molded',
            ],
        1032 =>
            [
                0 => 'moulder',
                1 => 'molder',
            ],
        1033 =>
            [
                0 => 'mouldered',
                1 => 'moldered',
            ],
        1034 =>
            [
                0 => 'mouldering',
                1 => 'moldering',
            ],
        1035 =>
            [
                0 => 'moulders',
                1 => 'molders',
            ],
        1036 =>
            [
                0 => 'mouldier',
                1 => 'moldier',
            ],
        1037 =>
            [
                0 => 'mouldiest',
                1 => 'moldiest',
            ],
        1038 =>
            [
                0 => 'moulding',
                1 => 'molding',
            ],
        1039 =>
            [
                0 => 'mouldings',
                1 => 'moldings',
            ],
        1040 =>
            [
                0 => 'moulds',
                1 => 'molds',
            ],
        1041 =>
            [
                0 => 'mouldy',
                1 => 'moldy',
            ],
        1042 =>
            [
                0 => 'moult',
                1 => 'molt',
            ],
        1043 =>
            [
                0 => 'moulted',
                1 => 'molted',
            ],
        1044 =>
            [
                0 => 'moulting',
                1 => 'molting',
            ],
        1045 =>
            [
                0 => 'moults',
                1 => 'molts',
            ],
        1046 =>
            [
                0 => 'moustache',
                1 => 'mustache',
            ],
        1047 =>
            [
                0 => 'moustached',
                1 => 'mustached',
            ],
        1048 =>
            [
                0 => 'moustaches',
                1 => 'mustaches',
            ],
        1049 =>
            [
                0 => 'moustachioed',
                1 => 'mustachioed',
            ],
        1050 =>
            [
                0 => 'multicoloured',
                1 => 'multicolored',
            ],
        1051 =>
            [
                0 => 'nationalisation',
                1 => 'nationalization',
            ],
        1052 =>
            [
                0 => 'nationalisations',
                1 => 'nationalizations',
            ],
        1053 =>
            [
                0 => 'nationalise',
                1 => 'nationalize',
            ],
        1054 =>
            [
                0 => 'nationalised',
                1 => 'nationalized',
            ],
        1055 =>
            [
                0 => 'nationalises',
                1 => 'nationalizes',
            ],
        1056 =>
            [
                0 => 'nationalising',
                1 => 'nationalizing',
            ],
        1057 =>
            [
                0 => 'naturalisation',
                1 => 'naturalization',
            ],
        1058 =>
            [
                0 => 'naturalise',
                1 => 'naturalize',
            ],
        1059 =>
            [
                0 => 'naturalised',
                1 => 'naturalized',
            ],
        1060 =>
            [
                0 => 'naturalises',
                1 => 'naturalizes',
            ],
        1061 =>
            [
                0 => 'naturalising',
                1 => 'naturalizing',
            ],
        1062 =>
            [
                0 => 'neighbour',
                1 => 'neighbor',
            ],
        1063 =>
            [
                0 => 'neighbourhood',
                1 => 'neighborhood',
            ],
        1064 =>
            [
                0 => 'neighbourhoods',
                1 => 'neighborhoods',
            ],
        1065 =>
            [
                0 => 'neighbouring',
                1 => 'neighboring',
            ],
        1066 =>
            [
                0 => 'neighbourliness',
                1 => 'neighborliness',
            ],
        1067 =>
            [
                0 => 'neighbourly',
                1 => 'neighborly',
            ],
        1068 =>
            [
                0 => 'neighbours',
                1 => 'neighbors',
            ],
        1069 =>
            [
                0 => 'neutralisation',
                1 => 'neutralization',
            ],
        1070 =>
            [
                0 => 'neutralise',
                1 => 'neutralize',
            ],
        1071 =>
            [
                0 => 'neutralised',
                1 => 'neutralized',
            ],
        1072 =>
            [
                0 => 'neutralises',
                1 => 'neutralizes',
            ],
        1073 =>
            [
                0 => 'neutralising',
                1 => 'neutralizing',
            ],
        1074 =>
            [
                0 => 'normalisation',
                1 => 'normalization',
            ],
        1075 =>
            [
                0 => 'normalise',
                1 => 'normalize',
            ],
        1076 =>
            [
                0 => 'normalised',
                1 => 'normalized',
            ],
        1077 =>
            [
                0 => 'normalises',
                1 => 'normalizes',
            ],
        1078 =>
            [
                0 => 'normalising',
                1 => 'normalizing',
            ],
        1079 =>
            [
                0 => 'odour',
                1 => 'odor',
            ],
        1080 =>
            [
                0 => 'odourless',
                1 => 'odorless',
            ],
        1081 =>
            [
                0 => 'odours',
                1 => 'odors',
            ],
        1082 =>
            [
                0 => 'oesophagus',
                1 => 'esophagus',
            ],
        1083 =>
            [
                0 => 'oesophaguses',
                1 => 'esophaguses',
            ],
        1084 =>
            [
                0 => 'oestrogen',
                1 => 'estrogen',
            ],
        1085 =>
            [
                0 => 'offence',
                1 => 'offense',
            ],
        1086 =>
            [
                0 => 'offences',
                1 => 'offenses',
            ],
        1087 =>
            [
                0 => 'omelette',
                1 => 'omelet',
            ],
        1088 =>
            [
                0 => 'omelettes',
                1 => 'omelets',
            ],
        1089 =>
            [
                0 => 'optimise',
                1 => 'optimize',
            ],
        1090 =>
            [
                0 => 'optimised',
                1 => 'optimized',
            ],
        1091 =>
            [
                0 => 'optimises',
                1 => 'optimizes',
            ],
        1092 =>
            [
                0 => 'optimising',
                1 => 'optimizing',
            ],
        1093 =>
            [
                0 => 'organisation',
                1 => 'organization',
            ],
        1094 =>
            [
                0 => 'organisational',
                1 => 'organizational',
            ],
        1095 =>
            [
                0 => 'organisations',
                1 => 'organizations',
            ],
        1096 =>
            [
                0 => 'organise',
                1 => 'organize',
            ],
        1097 =>
            [
                0 => 'organised',
                1 => 'organized',
            ],
        1098 =>
            [
                0 => 'organiser',
                1 => 'organizer',
            ],
        1099 =>
            [
                0 => 'organisers',
                1 => 'organizers',
            ],
        1100 =>
            [
                0 => 'organises',
                1 => 'organizes',
            ],
        1101 =>
            [
                0 => 'organising',
                1 => 'organizing',
            ],
        1102 =>
            [
                0 => 'orthopaedic',
                1 => 'orthopedic',
            ],
        1103 =>
            [
                0 => 'orthopaedics',
                1 => 'orthopedics',
            ],
        1104 =>
            [
                0 => 'ostracise',
                1 => 'ostracize',
            ],
        1105 =>
            [
                0 => 'ostracised',
                1 => 'ostracized',
            ],
        1106 =>
            [
                0 => 'ostracises',
                1 => 'ostracizes',
            ],
        1107 =>
            [
                0 => 'ostracising',
                1 => 'ostracizing',
            ],
        1108 =>
            [
                0 => 'outmanoeuvre',
                1 => 'outmaneuver',
            ],
        1109 =>
            [
                0 => 'outmanoeuvred',
                1 => 'outmaneuvered',
            ],
        1110 =>
            [
                0 => 'outmanoeuvres',
                1 => 'outmaneuvers',
            ],
        1111 =>
            [
                0 => 'outmanoeuvring',
                1 => 'outmaneuvering',
            ],
        1112 =>
            [
                0 => 'overemphasise',
                1 => 'overemphasize',
            ],
        1113 =>
            [
                0 => 'overemphasised',
                1 => 'overemphasized',
            ],
        1114 =>
            [
                0 => 'overemphasises',
                1 => 'overemphasizes',
            ],
        1115 =>
            [
                0 => 'overemphasising',
                1 => 'overemphasizing',
            ],
        1116 =>
            [
                0 => 'oxidisation',
                1 => 'oxidization',
            ],
        1117 =>
            [
                0 => 'oxidise',
                1 => 'oxidize',
            ],
        1118 =>
            [
                0 => 'oxidised',
                1 => 'oxidized',
            ],
        1119 =>
            [
                0 => 'oxidises',
                1 => 'oxidizes',
            ],
        1120 =>
            [
                0 => 'oxidising',
                1 => 'oxidizing',
            ],
        1121 =>
            [
                0 => 'paederast',
                1 => 'pederast',
            ],
        1122 =>
            [
                0 => 'paederasts',
                1 => 'pederasts',
            ],
        1123 =>
            [
                0 => 'paediatric',
                1 => 'pediatric',
            ],
        1124 =>
            [
                0 => 'paediatrician',
                1 => 'pediatrician',
            ],
        1125 =>
            [
                0 => 'paediatricians',
                1 => 'pediatricians',
            ],
        1126 =>
            [
                0 => 'paediatrics',
                1 => 'pediatrics',
            ],
        1127 =>
            [
                0 => 'paedophile',
                1 => 'pedophile',
            ],
        1128 =>
            [
                0 => 'paedophiles',
                1 => 'pedophiles',
            ],
        1129 =>
            [
                0 => 'paedophilia',
                1 => 'pedophilia',
            ],
        1130 =>
            [
                0 => 'palaeolithic',
                1 => 'paleolithic',
            ],
        1131 =>
            [
                0 => 'palaeontologist',
                1 => 'paleontologist',
            ],
        1132 =>
            [
                0 => 'palaeontologists',
                1 => 'paleontologists',
            ],
        1133 =>
            [
                0 => 'palaeontology',
                1 => 'paleontology',
            ],
        1134 =>
            [
                0 => 'panelled',
                1 => 'paneled',
            ],
        1135 =>
            [
                0 => 'panelling',
                1 => 'paneling',
            ],
        1136 =>
            [
                0 => 'panellist',
                1 => 'panelist',
            ],
        1137 =>
            [
                0 => 'panellists',
                1 => 'panelists',
            ],
        1138 =>
            [
                0 => 'paralyse',
                1 => 'paralyze',
            ],
        1139 =>
            [
                0 => 'paralysed',
                1 => 'paralyzed',
            ],
        1140 =>
            [
                0 => 'paralyses',
                1 => 'paralyzes',
            ],
        1141 =>
            [
                0 => 'paralysing',
                1 => 'paralyzing',
            ],
        1142 =>
            [
                0 => 'parcelled',
                1 => 'parceled',
            ],
        1143 =>
            [
                0 => 'parcelling',
                1 => 'parceling',
            ],
        1144 =>
            [
                0 => 'parlour',
                1 => 'parlor',
            ],
        1145 =>
            [
                0 => 'parlours',
                1 => 'parlors',
            ],
        1146 =>
            [
                0 => 'particularise',
                1 => 'particularize',
            ],
        1147 =>
            [
                0 => 'particularised',
                1 => 'particularized',
            ],
        1148 =>
            [
                0 => 'particularises',
                1 => 'particularizes',
            ],
        1149 =>
            [
                0 => 'particularising',
                1 => 'particularizing',
            ],
        1150 =>
            [
                0 => 'passivisation',
                1 => 'passivization',
            ],
        1151 =>
            [
                0 => 'passivise',
                1 => 'passivize',
            ],
        1152 =>
            [
                0 => 'passivised',
                1 => 'passivized',
            ],
        1153 =>
            [
                0 => 'passivises',
                1 => 'passivizes',
            ],
        1154 =>
            [
                0 => 'passivising',
                1 => 'passivizing',
            ],
        1155 =>
            [
                0 => 'pasteurisation',
                1 => 'pasteurization',
            ],
        1156 =>
            [
                0 => 'pasteurise',
                1 => 'pasteurize',
            ],
        1157 =>
            [
                0 => 'pasteurised',
                1 => 'pasteurized',
            ],
        1158 =>
            [
                0 => 'pasteurises',
                1 => 'pasteurizes',
            ],
        1159 =>
            [
                0 => 'pasteurising',
                1 => 'pasteurizing',
            ],
        1160 =>
            [
                0 => 'patronise',
                1 => 'patronize',
            ],
        1161 =>
            [
                0 => 'patronised',
                1 => 'patronized',
            ],
        1162 =>
            [
                0 => 'patronises',
                1 => 'patronizes',
            ],
        1163 =>
            [
                0 => 'patronising',
                1 => 'patronizing',
            ],
        1164 =>
            [
                0 => 'patronisingly',
                1 => 'patronizingly',
            ],
        1165 =>
            [
                0 => 'pedalled',
                1 => 'pedaled',
            ],
        1166 =>
            [
                0 => 'pedalling',
                1 => 'pedaling',
            ],
        1167 =>
            [
                0 => 'pedestrianisation',
                1 => 'pedestrianization',
            ],
        1168 =>
            [
                0 => 'pedestrianise',
                1 => 'pedestrianize',
            ],
        1169 =>
            [
                0 => 'pedestrianised',
                1 => 'pedestrianized',
            ],
        1170 =>
            [
                0 => 'pedestrianises',
                1 => 'pedestrianizes',
            ],
        1171 =>
            [
                0 => 'pedestrianising',
                1 => 'pedestrianizing',
            ],
        1172 =>
            [
                0 => 'penalise',
                1 => 'penalize',
            ],
        1173 =>
            [
                0 => 'penalised',
                1 => 'penalized',
            ],
        1174 =>
            [
                0 => 'penalises',
                1 => 'penalizes',
            ],
        1175 =>
            [
                0 => 'penalising',
                1 => 'penalizing',
            ],
        1176 =>
            [
                0 => 'pencilled',
                1 => 'penciled',
            ],
        1177 =>
            [
                0 => 'pencilling',
                1 => 'penciling',
            ],
        1178 =>
            [
                0 => 'personalise',
                1 => 'personalize',
            ],
        1179 =>
            [
                0 => 'personalised',
                1 => 'personalized',
            ],
        1180 =>
            [
                0 => 'personalises',
                1 => 'personalizes',
            ],
        1181 =>
            [
                0 => 'personalising',
                1 => 'personalizing',
            ],
        1182 =>
            [
                0 => 'pharmacopoeia',
                1 => 'pharmacopeia',
            ],
        1183 =>
            [
                0 => 'pharmacopoeias',
                1 => 'pharmacopeias',
            ],
        1184 =>
            [
                0 => 'philosophise',
                1 => 'philosophize',
            ],
        1185 =>
            [
                0 => 'philosophised',
                1 => 'philosophized',
            ],
        1186 =>
            [
                0 => 'philosophises',
                1 => 'philosophizes',
            ],
        1187 =>
            [
                0 => 'philosophising',
                1 => 'philosophizing',
            ],
        1188 =>
            [
                0 => 'philtre',
                1 => 'filter',
            ],
        1189 =>
            [
                0 => 'philtres',
                1 => 'filters',
            ],
        1190 =>
            [
                0 => 'phoney ',
                1 => 'phony ',
            ],
        1191 =>
            [
                0 => 'plagiarise',
                1 => 'plagiarize',
            ],
        1192 =>
            [
                0 => 'plagiarised',
                1 => 'plagiarized',
            ],
        1193 =>
            [
                0 => 'plagiarises',
                1 => 'plagiarizes',
            ],
        1194 =>
            [
                0 => 'plagiarising',
                1 => 'plagiarizing',
            ],
        1195 =>
            [
                0 => 'plough',
                1 => 'plow',
            ],
        1196 =>
            [
                0 => 'ploughed',
                1 => 'plowed',
            ],
        1197 =>
            [
                0 => 'ploughing',
                1 => 'plowing',
            ],
        1198 =>
            [
                0 => 'ploughman',
                1 => 'plowman',
            ],
        1199 =>
            [
                0 => 'ploughmen',
                1 => 'plowmen',
            ],
        1200 =>
            [
                0 => 'ploughs',
                1 => 'plows',
            ],
        1201 =>
            [
                0 => 'ploughshare',
                1 => 'plowshare',
            ],
        1202 =>
            [
                0 => 'ploughshares',
                1 => 'plowshares',
            ],
        1203 =>
            [
                0 => 'polarisation',
                1 => 'polarization',
            ],
        1204 =>
            [
                0 => 'polarise',
                1 => 'polarize',
            ],
        1205 =>
            [
                0 => 'polarised',
                1 => 'polarized',
            ],
        1206 =>
            [
                0 => 'polarises',
                1 => 'polarizes',
            ],
        1207 =>
            [
                0 => 'polarising',
                1 => 'polarizing',
            ],
        1208 =>
            [
                0 => 'politicisation',
                1 => 'politicization',
            ],
        1209 =>
            [
                0 => 'politicise',
                1 => 'politicize',
            ],
        1210 =>
            [
                0 => 'politicised',
                1 => 'politicized',
            ],
        1211 =>
            [
                0 => 'politicises',
                1 => 'politicizes',
            ],
        1212 =>
            [
                0 => 'politicising',
                1 => 'politicizing',
            ],
        1213 =>
            [
                0 => 'popularisation',
                1 => 'popularization',
            ],
        1214 =>
            [
                0 => 'popularise',
                1 => 'popularize',
            ],
        1215 =>
            [
                0 => 'popularised',
                1 => 'popularized',
            ],
        1216 =>
            [
                0 => 'popularises',
                1 => 'popularizes',
            ],
        1217 =>
            [
                0 => 'popularising',
                1 => 'popularizing',
            ],
        1218 =>
            [
                0 => 'pouffe',
                1 => 'pouf',
            ],
        1219 =>
            [
                0 => 'pouffes',
                1 => 'poufs',
            ],
        1220 =>
            [
                0 => 'practise',
                1 => 'practice',
            ],
        1221 =>
            [
                0 => 'practised',
                1 => 'practiced',
            ],
        1222 =>
            [
                0 => 'practises',
                1 => 'practices',
            ],
        1223 =>
            [
                0 => 'practising ',
                1 => 'practicing ',
            ],
        1224 =>
            [
                0 => 'praesidium',
                1 => 'presidium',
            ],
        1225 =>
            [
                0 => 'praesidiums ',
                1 => 'presidiums ',
            ],
        1226 =>
            [
                0 => 'pressurisation',
                1 => 'pressurization',
            ],
        1227 =>
            [
                0 => 'pressurise',
                1 => 'pressurize',
            ],
        1228 =>
            [
                0 => 'pressurised',
                1 => 'pressurized',
            ],
        1229 =>
            [
                0 => 'pressurises',
                1 => 'pressurizes',
            ],
        1230 =>
            [
                0 => 'pressurising',
                1 => 'pressurizing',
            ],
        1231 =>
            [
                0 => 'pretence',
                1 => 'pretense',
            ],
        1232 =>
            [
                0 => 'pretences',
                1 => 'pretenses',
            ],
        1233 =>
            [
                0 => 'primaeval',
                1 => 'primeval',
            ],
        1234 =>
            [
                0 => 'prioritisation',
                1 => 'prioritization',
            ],
        1235 =>
            [
                0 => 'prioritise',
                1 => 'prioritize',
            ],
        1236 =>
            [
                0 => 'prioritised',
                1 => 'prioritized',
            ],
        1237 =>
            [
                0 => 'prioritises',
                1 => 'prioritizes',
            ],
        1238 =>
            [
                0 => 'prioritising',
                1 => 'prioritizing',
            ],
        1239 =>
            [
                0 => 'privatisation',
                1 => 'privatization',
            ],
        1240 =>
            [
                0 => 'privatisations',
                1 => 'privatizations',
            ],
        1241 =>
            [
                0 => 'privatise',
                1 => 'privatize',
            ],
        1242 =>
            [
                0 => 'privatised',
                1 => 'privatized',
            ],
        1243 =>
            [
                0 => 'privatises',
                1 => 'privatizes',
            ],
        1244 =>
            [
                0 => 'privatising',
                1 => 'privatizing',
            ],
        1245 =>
            [
                0 => 'professionalisation',
                1 => 'professionalization',
            ],
        1246 =>
            [
                0 => 'professionalise',
                1 => 'professionalize',
            ],
        1247 =>
            [
                0 => 'professionalised',
                1 => 'professionalized',
            ],
        1248 =>
            [
                0 => 'professionalises',
                1 => 'professionalizes',
            ],
        1249 =>
            [
                0 => 'professionalising',
                1 => 'professionalizing',
            ],
        1250 =>
            [
                0 => 'programme',
                1 => 'program',
            ],
        1251 =>
            [
                0 => 'programmes',
                1 => 'programs',
            ],
        1252 =>
            [
                0 => 'prologue',
                1 => 'prolog',
            ],
        1253 =>
            [
                0 => 'prologues',
                1 => 'prologs',
            ],
        1254 =>
            [
                0 => 'propagandise',
                1 => 'propagandize',
            ],
        1255 =>
            [
                0 => 'propagandised',
                1 => 'propagandized',
            ],
        1256 =>
            [
                0 => 'propagandises',
                1 => 'propagandizes',
            ],
        1257 =>
            [
                0 => 'propagandising',
                1 => 'propagandizing',
            ],
        1258 =>
            [
                0 => 'proselytise',
                1 => 'proselytize',
            ],
        1259 =>
            [
                0 => 'proselytised',
                1 => 'proselytized',
            ],
        1260 =>
            [
                0 => 'proselytiser',
                1 => 'proselytizer',
            ],
        1261 =>
            [
                0 => 'proselytisers',
                1 => 'proselytizers',
            ],
        1262 =>
            [
                0 => 'proselytises',
                1 => 'proselytizes',
            ],
        1263 =>
            [
                0 => 'proselytising',
                1 => 'proselytizing',
            ],
        1264 =>
            [
                0 => 'psychoanalyse',
                1 => 'psychoanalyze',
            ],
        1265 =>
            [
                0 => 'psychoanalysed',
                1 => 'psychoanalyzed',
            ],
        1266 =>
            [
                0 => 'psychoanalyses',
                1 => 'psychoanalyzes',
            ],
        1267 =>
            [
                0 => 'psychoanalysing',
                1 => 'psychoanalyzing',
            ],
        1268 =>
            [
                0 => 'publicise',
                1 => 'publicize',
            ],
        1269 =>
            [
                0 => 'publicised',
                1 => 'publicized',
            ],
        1270 =>
            [
                0 => 'publicises',
                1 => 'publicizes',
            ],
        1271 =>
            [
                0 => 'publicising',
                1 => 'publicizing',
            ],
        1272 =>
            [
                0 => 'pulverisation',
                1 => 'pulverization',
            ],
        1273 =>
            [
                0 => 'pulverise',
                1 => 'pulverize',
            ],
        1274 =>
            [
                0 => 'pulverised',
                1 => 'pulverized',
            ],
        1275 =>
            [
                0 => 'pulverises',
                1 => 'pulverizes',
            ],
        1276 =>
            [
                0 => 'pulverising',
                1 => 'pulverizing',
            ],
        1277 =>
            [
                0 => 'pummelled',
                1 => 'pummel',
            ],
        1278 =>
            [
                0 => 'pummelling',
                1 => 'pummeled',
            ],
        1279 =>
            [
                0 => 'pyjama',
                1 => 'pajama',
            ],
        1280 =>
            [
                0 => 'pyjamas',
                1 => 'pajamas',
            ],
        1281 =>
            [
                0 => 'pzazz',
                1 => 'pizzazz',
            ],
        1282 =>
            [
                0 => 'quarrelled',
                1 => 'quarreled',
            ],
        1283 =>
            [
                0 => 'quarrelling',
                1 => 'quarreling',
            ],
        1284 =>
            [
                0 => 'radicalise',
                1 => 'radicalize',
            ],
        1285 =>
            [
                0 => 'radicalised',
                1 => 'radicalized',
            ],
        1286 =>
            [
                0 => 'radicalises',
                1 => 'radicalizes',
            ],
        1287 =>
            [
                0 => 'radicalising',
                1 => 'radicalizing',
            ],
        1288 =>
            [
                0 => 'rancour',
                1 => 'rancor',
            ],
        1289 =>
            [
                0 => 'randomise',
                1 => 'randomize',
            ],
        1290 =>
            [
                0 => 'randomised',
                1 => 'randomized',
            ],
        1291 =>
            [
                0 => 'randomises',
                1 => 'randomizes',
            ],
        1292 =>
            [
                0 => 'randomising',
                1 => 'randomizing',
            ],
        1293 =>
            [
                0 => 'rationalisation',
                1 => 'rationalization',
            ],
        1294 =>
            [
                0 => 'rationalisations',
                1 => 'rationalizations',
            ],
        1295 =>
            [
                0 => 'rationalise',
                1 => 'rationalize',
            ],
        1296 =>
            [
                0 => 'rationalised',
                1 => 'rationalized',
            ],
        1297 =>
            [
                0 => 'rationalises',
                1 => 'rationalizes',
            ],
        1298 =>
            [
                0 => 'rationalising',
                1 => 'rationalizing',
            ],
        1299 =>
            [
                0 => 'ravelled',
                1 => 'raveled',
            ],
        1300 =>
            [
                0 => 'ravelling',
                1 => 'raveling',
            ],
        1301 =>
            [
                0 => 'realisable',
                1 => 'realizable',
            ],
        1302 =>
            [
                0 => 'realisation',
                1 => 'realization',
            ],
        1303 =>
            [
                0 => 'realisations',
                1 => 'realizations',
            ],
        1304 =>
            [
                0 => 'realise',
                1 => 'realize',
            ],
        1305 =>
            [
                0 => 'realised',
                1 => 'realized',
            ],
        1306 =>
            [
                0 => 'realises',
                1 => 'realizes',
            ],
        1307 =>
            [
                0 => 'realising',
                1 => 'realizing',
            ],
        1308 =>
            [
                0 => 'recognisable',
                1 => 'recognizable',
            ],
        1309 =>
            [
                0 => 'recognisably',
                1 => 'recognizably',
            ],
        1310 =>
            [
                0 => 'recognisance',
                1 => 'recognizance',
            ],
        1311 =>
            [
                0 => 'recognise',
                1 => 'recognize',
            ],
        1312 =>
            [
                0 => 'recognised',
                1 => 'recognized',
            ],
        1313 =>
            [
                0 => 'recognises',
                1 => 'recognizes',
            ],
        1314 =>
            [
                0 => 'recognising',
                1 => 'recognizing',
            ],
        1315 =>
            [
                0 => 'reconnoitre',
                1 => 'reconnoiter',
            ],
        1316 =>
            [
                0 => 'reconnoitred',
                1 => 'reconnoitered',
            ],
        1317 =>
            [
                0 => 'reconnoitres',
                1 => 'reconnoiters',
            ],
        1318 =>
            [
                0 => 'reconnoitring',
                1 => 'reconnoitering',
            ],
        1319 =>
            [
                0 => 'refuelled',
                1 => 'refueled',
            ],
        1320 =>
            [
                0 => 'refuelling',
                1 => 'refueling',
            ],
        1321 =>
            [
                0 => 'regularisation',
                1 => 'regularization',
            ],
        1322 =>
            [
                0 => 'regularise',
                1 => 'regularize',
            ],
        1323 =>
            [
                0 => 'regularised',
                1 => 'regularized',
            ],
        1324 =>
            [
                0 => 'regularises',
                1 => 'regularizes',
            ],
        1325 =>
            [
                0 => 'regularising',
                1 => 'regularizing',
            ],
        1326 =>
            [
                0 => 'remodelled',
                1 => 'remodeled',
            ],
        1327 =>
            [
                0 => 'remodelling',
                1 => 'remodeling',
            ],
        1328 =>
            [
                0 => 'remould',
                1 => 'remold',
            ],
        1329 =>
            [
                0 => 'remoulded',
                1 => 'remolded',
            ],
        1330 =>
            [
                0 => 'remoulding',
                1 => 'remolding',
            ],
        1331 =>
            [
                0 => 'remoulds',
                1 => 'remolds',
            ],
        1332 =>
            [
                0 => 'reorganisation',
                1 => 'reorganization',
            ],
        1333 =>
            [
                0 => 'reorganisations',
                1 => 'reorganizations',
            ],
        1334 =>
            [
                0 => 'reorganise',
                1 => 'reorganize',
            ],
        1335 =>
            [
                0 => 'reorganised',
                1 => 'reorganized',
            ],
        1336 =>
            [
                0 => 'reorganises',
                1 => 'reorganizes',
            ],
        1337 =>
            [
                0 => 'reorganising',
                1 => 'reorganizing',
            ],
        1338 =>
            [
                0 => 'revelled',
                1 => 'reveled',
            ],
        1339 =>
            [
                0 => 'reveller',
                1 => 'reveler',
            ],
        1340 =>
            [
                0 => 'revellers',
                1 => 'revelers',
            ],
        1341 =>
            [
                0 => 'revelling',
                1 => 'reveling',
            ],
        1342 =>
            [
                0 => 'revitalise',
                1 => 'revitalize',
            ],
        1343 =>
            [
                0 => 'revitalised',
                1 => 'revitalized',
            ],
        1344 =>
            [
                0 => 'revitalises',
                1 => 'revitalizes',
            ],
        1345 =>
            [
                0 => 'revitalising',
                1 => 'revitalizing',
            ],
        1346 =>
            [
                0 => 'revolutionise',
                1 => 'revolutionize',
            ],
        1347 =>
            [
                0 => 'revolutionised',
                1 => 'revolutionized',
            ],
        1348 =>
            [
                0 => 'revolutionises',
                1 => 'revolutionizes',
            ],
        1349 =>
            [
                0 => 'revolutionising',
                1 => 'revolutionizing',
            ],
        1350 =>
            [
                0 => 'rhapsodise',
                1 => 'rhapsodize',
            ],
        1351 =>
            [
                0 => 'rhapsodised',
                1 => 'rhapsodized',
            ],
        1352 =>
            [
                0 => 'rhapsodises',
                1 => 'rhapsodizes',
            ],
        1353 =>
            [
                0 => 'rhapsodising',
                1 => 'rhapsodizing',
            ],
        1354 =>
            [
                0 => 'rigour',
                1 => 'rigor',
            ],
        1355 =>
            [
                0 => 'rigours',
                1 => 'rigors',
            ],
        1356 =>
            [
                0 => 'ritualised',
                1 => 'ritualized',
            ],
        1357 =>
            [
                0 => 'rivalled',
                1 => 'rivaled',
            ],
        1358 =>
            [
                0 => 'rivalling',
                1 => 'rivaling',
            ],
        1359 =>
            [
                0 => 'romanticise',
                1 => 'romanticize',
            ],
        1360 =>
            [
                0 => 'romanticised',
                1 => 'romanticized',
            ],
        1361 =>
            [
                0 => 'romanticises',
                1 => 'romanticizes',
            ],
        1362 =>
            [
                0 => 'romanticising',
                1 => 'romanticizing',
            ],
        1363 =>
            [
                0 => 'rumour',
                1 => 'rumor',
            ],
        1364 =>
            [
                0 => 'rumoured',
                1 => 'rumored',
            ],
        1365 =>
            [
                0 => 'rumours',
                1 => 'rumors',
            ],
        1366 =>
            [
                0 => 'sabre',
                1 => 'saber',
            ],
        1367 =>
            [
                0 => 'sabres',
                1 => 'sabers',
            ],
        1368 =>
            [
                0 => 'saltpetre',
                1 => 'saltpeter',
            ],
        1369 =>
            [
                0 => 'sanitise',
                1 => 'sanitize',
            ],
        1370 =>
            [
                0 => 'sanitised',
                1 => 'sanitized',
            ],
        1371 =>
            [
                0 => 'sanitises',
                1 => 'sanitizes',
            ],
        1372 =>
            [
                0 => 'sanitising',
                1 => 'sanitizing',
            ],
        1373 =>
            [
                0 => 'satirise',
                1 => 'satirize',
            ],
        1374 =>
            [
                0 => 'satirised',
                1 => 'satirized',
            ],
        1375 =>
            [
                0 => 'satirises',
                1 => 'satirizes',
            ],
        1376 =>
            [
                0 => 'satirising',
                1 => 'satirizing',
            ],
        1377 =>
            [
                0 => 'saviour',
                1 => 'savior',
            ],
        1378 =>
            [
                0 => 'saviours',
                1 => 'saviors',
            ],
        1379 =>
            [
                0 => 'savour',
                1 => 'savor',
            ],
        1380 =>
            [
                0 => 'savoured',
                1 => 'savored',
            ],
        1381 =>
            [
                0 => 'savouries',
                1 => 'savories',
            ],
        1382 =>
            [
                0 => 'savouring',
                1 => 'savoring',
            ],
        1383 =>
            [
                0 => 'savours',
                1 => 'savors',
            ],
        1384 =>
            [
                0 => 'savoury',
                1 => 'savory',
            ],
        1385 =>
            [
                0 => 'scandalise',
                1 => 'scandalize',
            ],
        1386 =>
            [
                0 => 'scandalised',
                1 => 'scandalized',
            ],
        1387 =>
            [
                0 => 'scandalises',
                1 => 'scandalizes',
            ],
        1388 =>
            [
                0 => 'scandalising',
                1 => 'scandalizing',
            ],
        1389 =>
            [
                0 => 'sceptic',
                1 => 'skeptic',
            ],
        1390 =>
            [
                0 => 'sceptical',
                1 => 'skeptical',
            ],
        1391 =>
            [
                0 => 'sceptically',
                1 => 'skeptically',
            ],
        1392 =>
            [
                0 => 'scepticism',
                1 => 'skepticism',
            ],
        1393 =>
            [
                0 => 'sceptics',
                1 => 'skeptics',
            ],
        1394 =>
            [
                0 => 'sceptre',
                1 => 'scepter',
            ],
        1395 =>
            [
                0 => 'sceptres',
                1 => 'scepters',
            ],
        1396 =>
            [
                0 => 'scrutinise',
                1 => 'scrutinize',
            ],
        1397 =>
            [
                0 => 'scrutinised',
                1 => 'scrutinized',
            ],
        1398 =>
            [
                0 => 'scrutinises',
                1 => 'scrutinizes',
            ],
        1399 =>
            [
                0 => 'scrutinising',
                1 => 'scrutinizing',
            ],
        1400 =>
            [
                0 => 'secularisation',
                1 => 'secularization',
            ],
        1401 =>
            [
                0 => 'secularise',
                1 => 'secularize',
            ],
        1402 =>
            [
                0 => 'secularised',
                1 => 'secularized',
            ],
        1403 =>
            [
                0 => 'secularises',
                1 => 'secularizes',
            ],
        1404 =>
            [
                0 => 'secularising',
                1 => 'secularizing',
            ],
        1405 =>
            [
                0 => 'sensationalise',
                1 => 'sensationalize',
            ],
        1406 =>
            [
                0 => 'sensationalised',
                1 => 'sensationalized',
            ],
        1407 =>
            [
                0 => 'sensationalises',
                1 => 'sensationalizes',
            ],
        1408 =>
            [
                0 => 'sensationalising',
                1 => 'sensationalizing',
            ],
        1409 =>
            [
                0 => 'sensitise',
                1 => 'sensitize',
            ],
        1410 =>
            [
                0 => 'sensitised',
                1 => 'sensitized',
            ],
        1411 =>
            [
                0 => 'sensitises',
                1 => 'sensitizes',
            ],
        1412 =>
            [
                0 => 'sensitising',
                1 => 'sensitizing',
            ],
        1413 =>
            [
                0 => 'sentimentalise',
                1 => 'sentimentalize',
            ],
        1414 =>
            [
                0 => 'sentimentalised',
                1 => 'sentimentalized',
            ],
        1415 =>
            [
                0 => 'sentimentalises',
                1 => 'sentimentalizes',
            ],
        1416 =>
            [
                0 => 'sentimentalising',
                1 => 'sentimentalizing',
            ],
        1417 =>
            [
                0 => 'sepulchre',
                1 => 'sepulcher',
            ],
        1418 =>
            [
                0 => 'sepulchres',
                1 => 'sepulchers ',
            ],
        1419 =>
            [
                0 => 'serialisation',
                1 => 'serialization',
            ],
        1420 =>
            [
                0 => 'serialisations',
                1 => 'serializations',
            ],
        1421 =>
            [
                0 => 'serialise',
                1 => 'serialize',
            ],
        1422 =>
            [
                0 => 'serialised',
                1 => 'serialized',
            ],
        1423 =>
            [
                0 => 'serialises',
                1 => 'serializes',
            ],
        1424 =>
            [
                0 => 'serialising',
                1 => 'serializing',
            ],
        1425 =>
            [
                0 => 'sermonise',
                1 => 'sermonize',
            ],
        1426 =>
            [
                0 => 'sermonised',
                1 => 'sermonized',
            ],
        1427 =>
            [
                0 => 'sermonises',
                1 => 'sermonizes',
            ],
        1428 =>
            [
                0 => 'sermonising',
                1 => 'sermonizing',
            ],
        1429 =>
            [
                0 => 'sheikh ',
                1 => 'sheik ',
            ],
        1430 =>
            [
                0 => 'shovelled',
                1 => 'shoveled',
            ],
        1431 =>
            [
                0 => 'shovelling',
                1 => 'shoveling',
            ],
        1432 =>
            [
                0 => 'shrivelled',
                1 => 'shriveled',
            ],
        1433 =>
            [
                0 => 'shrivelling',
                1 => 'shriveling',
            ],
        1434 =>
            [
                0 => 'signalise',
                1 => 'signalize',
            ],
        1435 =>
            [
                0 => 'signalised',
                1 => 'signalized',
            ],
        1436 =>
            [
                0 => 'signalises',
                1 => 'signalizes',
            ],
        1437 =>
            [
                0 => 'signalising',
                1 => 'signalizing',
            ],
        1438 =>
            [
                0 => 'signalled',
                1 => 'signaled',
            ],
        1439 =>
            [
                0 => 'signalling',
                1 => 'signaling',
            ],
        1440 =>
            [
                0 => 'smoulder',
                1 => 'smolder',
            ],
        1441 =>
            [
                0 => 'smouldered',
                1 => 'smoldered',
            ],
        1442 =>
            [
                0 => 'smouldering',
                1 => 'smoldering',
            ],
        1443 =>
            [
                0 => 'smoulders',
                1 => 'smolders',
            ],
        1444 =>
            [
                0 => 'snivelled',
                1 => 'sniveled',
            ],
        1445 =>
            [
                0 => 'snivelling',
                1 => 'sniveling',
            ],
        1446 =>
            [
                0 => 'snorkelled',
                1 => 'snorkeled',
            ],
        1447 =>
            [
                0 => 'snorkelling',
                1 => 'snorkeling',
            ],
        1448 =>
            [
                0 => 'snowplough',
                1 => 'snowplow',
            ],
        1449 =>
            [
                0 => 'snowploughs',
                1 => 'snowplow',
            ],
        1450 =>
            [
                0 => 'socialisation',
                1 => 'socialization',
            ],
        1451 =>
            [
                0 => 'socialise',
                1 => 'socialize',
            ],
        1452 =>
            [
                0 => 'socialised',
                1 => 'socialized',
            ],
        1453 =>
            [
                0 => 'socialises',
                1 => 'socializes',
            ],
        1454 =>
            [
                0 => 'socialising',
                1 => 'socializing',
            ],
        1455 =>
            [
                0 => 'sodomise',
                1 => 'sodomize',
            ],
        1456 =>
            [
                0 => 'sodomised',
                1 => 'sodomized',
            ],
        1457 =>
            [
                0 => 'sodomises',
                1 => 'sodomizes',
            ],
        1458 =>
            [
                0 => 'sodomising',
                1 => 'sodomizing',
            ],
        1459 =>
            [
                0 => 'solemnise',
                1 => 'solemnize',
            ],
        1460 =>
            [
                0 => 'solemnised',
                1 => 'solemnized',
            ],
        1461 =>
            [
                0 => 'solemnises',
                1 => 'solemnizes',
            ],
        1462 =>
            [
                0 => 'solemnising',
                1 => 'solemnizing',
            ],
        1463 =>
            [
                0 => 'sombre',
                1 => 'somber',
            ],
        1464 =>
            [
                0 => 'specialisation',
                1 => 'specialization',
            ],
        1465 =>
            [
                0 => 'specialisations',
                1 => 'specializations',
            ],
        1466 =>
            [
                0 => 'specialise',
                1 => 'specialize',
            ],
        1467 =>
            [
                0 => 'specialised',
                1 => 'specialized',
            ],
        1468 =>
            [
                0 => 'specialises',
                1 => 'specializes',
            ],
        1469 =>
            [
                0 => 'specialising',
                1 => 'specializing',
            ],
        1470 =>
            [
                0 => 'spectre',
                1 => 'specter',
            ],
        1471 =>
            [
                0 => 'spectres',
                1 => 'specters',
            ],
        1472 =>
            [
                0 => 'spiralled',
                1 => 'spiraled',
            ],
        1473 =>
            [
                0 => 'spiralling',
                1 => 'spiraling',
            ],
        1474 =>
            [
                0 => 'splendour',
                1 => 'splendor',
            ],
        1475 =>
            [
                0 => 'splendours',
                1 => 'splendors',
            ],
        1476 =>
            [
                0 => 'squirrelled',
                1 => 'squirreled',
            ],
        1477 =>
            [
                0 => 'squirrelling',
                1 => 'squirreling',
            ],
        1478 =>
            [
                0 => 'stabilisation',
                1 => 'stabilization',
            ],
        1479 =>
            [
                0 => 'stabilise',
                1 => 'stabilize',
            ],
        1480 =>
            [
                0 => 'stabilised',
                1 => 'stabilized',
            ],
        1481 =>
            [
                0 => 'stabiliser',
                1 => 'stabilizer',
            ],
        1482 =>
            [
                0 => 'stabilisers',
                1 => 'stabilizers',
            ],
        1483 =>
            [
                0 => 'stabilises',
                1 => 'stabilizes',
            ],
        1484 =>
            [
                0 => 'stabilising',
                1 => 'stabilizing',
            ],
        1485 =>
            [
                0 => 'standardisation',
                1 => 'standardization',
            ],
        1486 =>
            [
                0 => 'standardise',
                1 => 'standardize',
            ],
        1487 =>
            [
                0 => 'standardised',
                1 => 'standardized',
            ],
        1488 =>
            [
                0 => 'standardises',
                1 => 'standardizes',
            ],
        1489 =>
            [
                0 => 'standardising',
                1 => 'standardizing',
            ],
        1490 =>
            [
                0 => 'stencilled',
                1 => 'stenciled',
            ],
        1491 =>
            [
                0 => 'stencilling',
                1 => 'stenciling',
            ],
        1492 =>
            [
                0 => 'sterilisation',
                1 => 'sterilization',
            ],
        1493 =>
            [
                0 => 'sterilisations',
                1 => 'sterilizations',
            ],
        1494 =>
            [
                0 => 'sterilise',
                1 => 'sterilize',
            ],
        1495 =>
            [
                0 => 'sterilised',
                1 => 'sterilized',
            ],
        1496 =>
            [
                0 => 'steriliser',
                1 => 'sterilizer',
            ],
        1497 =>
            [
                0 => 'sterilisers',
                1 => 'sterilizers',
            ],
        1498 =>
            [
                0 => 'sterilises',
                1 => 'sterilizes',
            ],
        1499 =>
            [
                0 => 'sterilising',
                1 => 'sterilizing',
            ],
        1500 =>
            [
                0 => 'stigmatisation',
                1 => 'stigmatization',
            ],
        1501 =>
            [
                0 => 'stigmatise',
                1 => 'stigmatize',
            ],
        1502 =>
            [
                0 => 'stigmatised',
                1 => 'stigmatized',
            ],
        1503 =>
            [
                0 => 'stigmatises',
                1 => 'stigmatizes',
            ],
        1504 =>
            [
                0 => 'stigmatising',
                1 => 'stigmatizing',
            ],
        1505 =>
            [
                0 => 'storey',
                1 => 'story',
            ],
        1506 =>
            [
                0 => 'storeys',
                1 => 'stories',
            ],
        1507 =>
            [
                0 => 'subsidisation',
                1 => 'subsidization',
            ],
        1508 =>
            [
                0 => 'subsidise',
                1 => 'subsidize',
            ],
        1509 =>
            [
                0 => 'subsidised',
                1 => 'subsidized',
            ],
        1510 =>
            [
                0 => 'subsidiser',
                1 => 'subsidizer',
            ],
        1511 =>
            [
                0 => 'subsidisers',
                1 => 'subsidizers',
            ],
        1512 =>
            [
                0 => 'subsidises',
                1 => 'subsidizes',
            ],
        1513 =>
            [
                0 => 'subsidising',
                1 => 'subsidizing',
            ],
        1514 =>
            [
                0 => 'succour',
                1 => 'succor',
            ],
        1515 =>
            [
                0 => 'succoured',
                1 => 'succored',
            ],
        1516 =>
            [
                0 => 'succouring',
                1 => 'succoring',
            ],
        1517 =>
            [
                0 => 'succours',
                1 => 'succors',
            ],
        1518 =>
            [
                0 => 'sulphate',
                1 => 'sulfate',
            ],
        1519 =>
            [
                0 => 'sulphates',
                1 => 'sulfates',
            ],
        1520 =>
            [
                0 => 'sulphide',
                1 => 'sulfide',
            ],
        1521 =>
            [
                0 => 'sulphides',
                1 => 'sulfides',
            ],
        1522 =>
            [
                0 => 'sulphur',
                1 => 'sulfur',
            ],
        1523 =>
            [
                0 => 'sulphurous',
                1 => 'sulfurous',
            ],
        1524 =>
            [
                0 => 'summarise',
                1 => 'summarize',
            ],
        1525 =>
            [
                0 => 'summarised',
                1 => 'summarized',
            ],
        1526 =>
            [
                0 => 'summarises',
                1 => 'summarizes',
            ],
        1527 =>
            [
                0 => 'summarising',
                1 => 'summarizing',
            ],
        1528 =>
            [
                0 => 'swivelled',
                1 => 'swiveled',
            ],
        1529 =>
            [
                0 => 'swivelling',
                1 => 'swiveling',
            ],
        1530 =>
            [
                0 => 'symbolise',
                1 => 'symbolize',
            ],
        1531 =>
            [
                0 => 'symbolised',
                1 => 'symbolized',
            ],
        1532 =>
            [
                0 => 'symbolises',
                1 => 'symbolizes',
            ],
        1533 =>
            [
                0 => 'symbolising',
                1 => 'symbolizing',
            ],
        1534 =>
            [
                0 => 'sympathise',
                1 => 'sympathize',
            ],
        1535 =>
            [
                0 => 'sympathised',
                1 => 'sympathized',
            ],
        1536 =>
            [
                0 => 'sympathiser',
                1 => 'sympathizer',
            ],
        1537 =>
            [
                0 => 'sympathisers',
                1 => 'sympathizers',
            ],
        1538 =>
            [
                0 => 'sympathises',
                1 => 'sympathizes',
            ],
        1539 =>
            [
                0 => 'sympathising',
                1 => 'sympathizing',
            ],
        1540 =>
            [
                0 => 'synchronisation',
                1 => 'synchronization',
            ],
        1541 =>
            [
                0 => 'synchronise',
                1 => 'synchronize',
            ],
        1542 =>
            [
                0 => 'synchronised',
                1 => 'synchronized',
            ],
        1543 =>
            [
                0 => 'synchronises',
                1 => 'synchronizes',
            ],
        1544 =>
            [
                0 => 'synchronising',
                1 => 'synchronizing',
            ],
        1545 =>
            [
                0 => 'synthesise',
                1 => 'synthesize',
            ],
        1546 =>
            [
                0 => 'synthesised',
                1 => 'synthesized',
            ],
        1547 =>
            [
                0 => 'synthesiser',
                1 => 'synthesizer',
            ],
        1548 =>
            [
                0 => 'synthesisers',
                1 => 'synthesizers',
            ],
        1549 =>
            [
                0 => 'synthesises',
                1 => 'synthesizes',
            ],
        1550 =>
            [
                0 => 'synthesising',
                1 => 'synthesizing',
            ],
        1551 =>
            [
                0 => 'syphon',
                1 => 'siphon',
            ],
        1552 =>
            [
                0 => 'syphoned',
                1 => 'siphoned',
            ],
        1553 =>
            [
                0 => 'syphoning',
                1 => 'siphoning',
            ],
        1554 =>
            [
                0 => 'syphons',
                1 => 'siphons',
            ],
        1555 =>
            [
                0 => 'systematisation',
                1 => 'systematization',
            ],
        1556 =>
            [
                0 => 'systematise',
                1 => 'systematize',
            ],
        1557 =>
            [
                0 => 'systematised',
                1 => 'systematized',
            ],
        1558 =>
            [
                0 => 'systematises',
                1 => 'systematizes',
            ],
        1559 =>
            [
                0 => 'systematising',
                1 => 'systematizing',
            ],
        1560 =>
            [
                0 => 'tantalise',
                1 => 'tantalize',
            ],
        1561 =>
            [
                0 => 'tantalised',
                1 => 'tantalized',
            ],
        1562 =>
            [
                0 => 'tantalises',
                1 => 'tantalizes',
            ],
        1563 =>
            [
                0 => 'tantalising',
                1 => 'tantalizing',
            ],
        1564 =>
            [
                0 => 'tantalisingly',
                1 => 'tantalizingly',
            ],
        1565 =>
            [
                0 => 'tasselled',
                1 => 'tasseled',
            ],
        1566 =>
            [
                0 => 'technicolour',
                1 => 'technicolor',
            ],
        1567 =>
            [
                0 => 'temporise',
                1 => 'temporize',
            ],
        1568 =>
            [
                0 => 'temporised',
                1 => 'temporized',
            ],
        1569 =>
            [
                0 => 'temporises',
                1 => 'temporizes',
            ],
        1570 =>
            [
                0 => 'temporising',
                1 => 'temporizing',
            ],
        1571 =>
            [
                0 => 'tenderise',
                1 => 'tenderize',
            ],
        1572 =>
            [
                0 => 'tenderised',
                1 => 'tenderized',
            ],
        1573 =>
            [
                0 => 'tenderises',
                1 => 'tenderizes',
            ],
        1574 =>
            [
                0 => 'tenderising',
                1 => 'tenderizing',
            ],
        1575 =>
            [
                0 => 'terrorise',
                1 => 'terrorize',
            ],
        1576 =>
            [
                0 => 'terrorised',
                1 => 'terrorized',
            ],
        1577 =>
            [
                0 => 'terrorises',
                1 => 'terrorizes',
            ],
        1578 =>
            [
                0 => 'terrorising',
                1 => 'terrorizing',
            ],
        1579 =>
            [
                0 => 'theatre',
                1 => 'theater',
            ],
        1580 =>
            [
                0 => 'theatregoer',
                1 => 'theatergoer',
            ],
        1581 =>
            [
                0 => 'theatregoers',
                1 => 'theatergoers',
            ],
        1582 =>
            [
                0 => 'theatres',
                1 => 'theaters',
            ],
        1583 =>
            [
                0 => 'theorise',
                1 => 'theorize',
            ],
        1584 =>
            [
                0 => 'theorised',
                1 => 'theorized',
            ],
        1585 =>
            [
                0 => 'theorises',
                1 => 'theorizes',
            ],
        1586 =>
            [
                0 => 'theorising',
                1 => 'theorizing',
            ],
        1587 =>
            [
                0 => 'tonne',
                1 => 'ton',
            ],
        1588 =>
            [
                0 => 'tonnes',
                1 => 'tons',
            ],
        1589 =>
            [
                0 => 'towelled',
                1 => 'toweled',
            ],
        1590 =>
            [
                0 => 'towelling',
                1 => 'toweling',
            ],
        1591 =>
            [
                0 => 'toxaemia',
                1 => 'toxemia',
            ],
        1592 =>
            [
                0 => 'tranquillise',
                1 => 'tranquilize',
            ],
        1593 =>
            [
                0 => 'tranquillised',
                1 => 'tranquilized',
            ],
        1594 =>
            [
                0 => 'tranquilliser',
                1 => 'tranquilizer',
            ],
        1595 =>
            [
                0 => 'tranquillisers',
                1 => 'tranquilizers',
            ],
        1596 =>
            [
                0 => 'tranquillises',
                1 => 'tranquilizes',
            ],
        1597 =>
            [
                0 => 'tranquillising',
                1 => 'tranquilizing',
            ],
        1598 =>
            [
                0 => 'tranquillity',
                1 => 'tranquility',
            ],
        1599 =>
            [
                0 => 'tranquillize',
                1 => 'tranquilize',
            ],
        1600 =>
            [
                0 => 'tranquillized',
                1 => 'tranquilized',
            ],
        1601 =>
            [
                0 => 'tranquillizer',
                1 => 'tranquilizer',
            ],
        1602 =>
            [
                0 => 'tranquillizers',
                1 => 'tranquilizers',
            ],
        1603 =>
            [
                0 => 'tranquillizes',
                1 => 'tranquilizes',
            ],
        1604 =>
            [
                0 => 'tranquillizing',
                1 => 'tranquilizing',
            ],
        1605 =>
            [
                0 => 'tranquilly',
                1 => 'tranquility',
            ],
        1606 =>
            [
                0 => 'transistorised',
                1 => 'transistorized',
            ],
        1607 =>
            [
                0 => 'traumatise',
                1 => 'traumatize',
            ],
        1608 =>
            [
                0 => 'traumatised',
                1 => 'traumatized',
            ],
        1609 =>
            [
                0 => 'traumatises',
                1 => 'traumatizes',
            ],
        1610 =>
            [
                0 => 'traumatising',
                1 => 'traumatizing',
            ],
        1611 =>
            [
                0 => 'travelled',
                1 => 'traveled',
            ],
        1612 =>
            [
                0 => 'traveller',
                1 => 'traveler',
            ],
        1613 =>
            [
                0 => 'travellers',
                1 => 'travelers',
            ],
        1614 =>
            [
                0 => 'travelling',
                1 => 'traveling',
            ],
        1615 =>
            [
                0 => 'travelogue',
                1 => 'travelog',
            ],
        1616 =>
            [
                0 => 'travelogues ',
                1 => 'travelogs ',
            ],
        1617 =>
            [
                0 => 'trialled',
                1 => 'trialed',
            ],
        1618 =>
            [
                0 => 'trialling',
                1 => 'trialing',
            ],
        1619 =>
            [
                0 => 'tricolour',
                1 => 'tricolor',
            ],
        1620 =>
            [
                0 => 'tricolours',
                1 => 'tricolors',
            ],
        1621 =>
            [
                0 => 'trivialise',
                1 => 'trivialize',
            ],
        1622 =>
            [
                0 => 'trivialised',
                1 => 'trivialized',
            ],
        1623 =>
            [
                0 => 'trivialises',
                1 => 'trivializes',
            ],
        1624 =>
            [
                0 => 'trivialising',
                1 => 'trivializing',
            ],
        1625 =>
            [
                0 => 'tumour',
                1 => 'tumor',
            ],
        1626 =>
            [
                0 => 'tumours',
                1 => 'tumors',
            ],
        1627 =>
            [
                0 => 'tunnelled',
                1 => 'tunneled',
            ],
        1628 =>
            [
                0 => 'tunnelling',
                1 => 'tunneling',
            ],
        1629 =>
            [
                0 => 'tyrannise',
                1 => 'tyrannize',
            ],
        1630 =>
            [
                0 => 'tyrannised',
                1 => 'tyrannized',
            ],
        1631 =>
            [
                0 => 'tyrannises',
                1 => 'tyrannizes',
            ],
        1632 =>
            [
                0 => 'tyrannising',
                1 => 'tyrannizing',
            ],
        1633 =>
            [
                0 => 'tyre',
                1 => 'tire',
            ],
        1634 =>
            [
                0 => 'tyres',
                1 => 'tires',
            ],
        1635 =>
            [
                0 => 'unauthorised',
                1 => 'unauthorized',
            ],
        1636 =>
            [
                0 => 'uncivilised',
                1 => 'uncivilized',
            ],
        1637 =>
            [
                0 => 'underutilised',
                1 => 'underutilized',
            ],
        1638 =>
            [
                0 => 'unequalled',
                1 => 'unequaled',
            ],
        1639 =>
            [
                0 => 'unfavourable',
                1 => 'unfavorable',
            ],
        1640 =>
            [
                0 => 'unfavourably',
                1 => 'unfavorably',
            ],
        1641 =>
            [
                0 => 'unionisation',
                1 => 'unionization',
            ],
        1642 =>
            [
                0 => 'unionise',
                1 => 'unionize',
            ],
        1643 =>
            [
                0 => 'unionised',
                1 => 'unionized',
            ],
        1644 =>
            [
                0 => 'unionises',
                1 => 'unionizes',
            ],
        1645 =>
            [
                0 => 'unionising',
                1 => 'unionizing',
            ],
        1646 =>
            [
                0 => 'unorganised',
                1 => 'unorganized',
            ],
        1647 =>
            [
                0 => 'unravelled',
                1 => 'unraveled',
            ],
        1648 =>
            [
                0 => 'unravelling',
                1 => 'unraveling',
            ],
        1649 =>
            [
                0 => 'unrecognisable',
                1 => 'unrecognizable',
            ],
        1650 =>
            [
                0 => 'unrecognised',
                1 => 'unrecognized',
            ],
        1651 =>
            [
                0 => 'unrivalled',
                1 => 'unrivaled',
            ],
        1652 =>
            [
                0 => 'unsavoury',
                1 => 'unsavory',
            ],
        1653 =>
            [
                0 => 'untrammelled',
                1 => 'untrammeled',
            ],
        1654 =>
            [
                0 => 'urbanisation',
                1 => 'urbanization',
            ],
        1655 =>
            [
                0 => 'urbanise',
                1 => 'urbanize',
            ],
        1656 =>
            [
                0 => 'urbanised',
                1 => 'urbanized',
            ],
        1657 =>
            [
                0 => 'urbanises',
                1 => 'urbanizes',
            ],
        1658 =>
            [
                0 => 'urbanising',
                1 => 'urbanizing',
            ],
        1659 =>
            [
                0 => 'utilisable',
                1 => 'utilizable',
            ],
        1660 =>
            [
                0 => 'utilisation',
                1 => 'utilization',
            ],
        1661 =>
            [
                0 => 'utilise',
                1 => 'utilize',
            ],
        1662 =>
            [
                0 => 'utilised',
                1 => 'utilized',
            ],
        1663 =>
            [
                0 => 'utilises',
                1 => 'utilizes',
            ],
        1664 =>
            [
                0 => 'utilising',
                1 => 'utilizing',
            ],
        1665 =>
            [
                0 => 'valour',
                1 => 'valor',
            ],
        1666 =>
            [
                0 => 'vandalise',
                1 => 'vandalize',
            ],
        1667 =>
            [
                0 => 'vandalised',
                1 => 'vandalized',
            ],
        1668 =>
            [
                0 => 'vandalises',
                1 => 'vandalizes',
            ],
        1669 =>
            [
                0 => 'vandalising',
                1 => 'vandalizing',
            ],
        1670 =>
            [
                0 => 'vaporisation',
                1 => 'vaporization',
            ],
        1671 =>
            [
                0 => 'vaporise',
                1 => 'vaporize',
            ],
        1672 =>
            [
                0 => 'vaporised',
                1 => 'vaporized',
            ],
        1673 =>
            [
                0 => 'vaporises',
                1 => 'vaporizes',
            ],
        1674 =>
            [
                0 => 'vaporising',
                1 => 'vaporizing',
            ],
        1675 =>
            [
                0 => 'vapour',
                1 => 'vapor',
            ],
        1676 =>
            [
                0 => 'vapours',
                1 => 'vapors',
            ],
        1677 =>
            [
                0 => 'verbalise',
                1 => 'verbalize',
            ],
        1678 =>
            [
                0 => 'verbalised',
                1 => 'verbalized',
            ],
        1679 =>
            [
                0 => 'verbalises',
                1 => 'verbalizes',
            ],
        1680 =>
            [
                0 => 'verbalising',
                1 => 'verbalizing',
            ],
        1681 =>
            [
                0 => 'victimisation',
                1 => 'victimization',
            ],
        1682 =>
            [
                0 => 'victimise',
                1 => 'victimize',
            ],
        1683 =>
            [
                0 => 'victimised',
                1 => 'victimized',
            ],
        1684 =>
            [
                0 => 'victimises',
                1 => 'victimizes',
            ],
        1685 =>
            [
                0 => 'victimising',
                1 => 'victimizing',
            ],
        1686 =>
            [
                0 => 'videodisc',
                1 => 'videodisk',
            ],
        1687 =>
            [
                0 => 'videodiscs',
                1 => 'videodisks',
            ],
        1688 =>
            [
                0 => 'vigour',
                1 => 'vigor',
            ],
        1689 =>
            [
                0 => 'visualisation',
                1 => 'visualization',
            ],
        1690 =>
            [
                0 => 'visualisations',
                1 => 'visualizations',
            ],
        1691 =>
            [
                0 => 'visualise',
                1 => 'visualize',
            ],
        1692 =>
            [
                0 => 'visualised',
                1 => 'visualized',
            ],
        1693 =>
            [
                0 => 'visualises',
                1 => 'visualizes',
            ],
        1694 =>
            [
                0 => 'visualising',
                1 => 'visualizing',
            ],
        1695 =>
            [
                0 => 'vocalisation',
                1 => 'vocalization',
            ],
        1696 =>
            [
                0 => 'vocalisations',
                1 => 'vocalizations',
            ],
        1697 =>
            [
                0 => 'vocalise',
                1 => 'vocalize',
            ],
        1698 =>
            [
                0 => 'vocalised',
                1 => 'vocalized',
            ],
        1699 =>
            [
                0 => 'vocalises',
                1 => 'vocalizes',
            ],
        1700 =>
            [
                0 => 'vocalising',
                1 => 'vocalizing',
            ],
        1701 =>
            [
                0 => 'vulcanised',
                1 => 'vulcanized',
            ],
        1702 =>
            [
                0 => 'vulgarisation',
                1 => 'vulgarization',
            ],
        1703 =>
            [
                0 => 'vulgarise',
                1 => 'vulgarize',
            ],
        1704 =>
            [
                0 => 'vulgarised',
                1 => 'vulgarized',
            ],
        1705 =>
            [
                0 => 'vulgarises',
                1 => 'vulgarizes',
            ],
        1706 =>
            [
                0 => 'vulgarising',
                1 => 'vulgarizing',
            ],
        1707 =>
            [
                0 => 'waggon',
                1 => 'wagon',
            ],
        1708 =>
            [
                0 => 'waggons',
                1 => 'wagons',
            ],
        1709 =>
            [
                0 => 'watercolour',
                1 => 'watercolor',
            ],
        1710 =>
            [
                0 => 'watercolours',
                1 => 'watercolors',
            ],
        1711 =>
            [
                0 => 'weaselled',
                1 => 'weaseled',
            ],
        1712 =>
            [
                0 => 'weaselling',
                1 => 'weaseling',
            ],
        1713 =>
            [
                0 => 'westernisation',
                1 => 'westernization',
            ],
        1714 =>
            [
                0 => 'westernise',
                1 => 'westernize',
            ],
        1715 =>
            [
                0 => 'westernised',
                1 => 'westernized',
            ],
        1716 =>
            [
                0 => 'westernises',
                1 => 'westernizes',
            ],
        1717 =>
            [
                0 => 'westernising',
                1 => 'westernizing',
            ],
        1718 =>
            [
                0 => 'womanise',
                1 => 'womanize',
            ],
        1719 =>
            [
                0 => 'womanised',
                1 => 'womanized',
            ],
        1720 =>
            [
                0 => 'womaniser',
                1 => 'womanizer',
            ],
        1721 =>
            [
                0 => 'womanisers',
                1 => 'womanizers',
            ],
        1722 =>
            [
                0 => 'womanises',
                1 => 'womanizes',
            ],
        1723 =>
            [
                0 => 'womanising',
                1 => 'womanizing',
            ],
        1724 =>
            [
                0 => 'woollen',
                1 => 'woolen',
            ],
        1725 =>
            [
                0 => 'woollens',
                1 => 'woolens',
            ],
        1726 =>
            [
                0 => 'woollies',
                1 => 'woolies',
            ],
        1727 =>
            [
                0 => 'woolly',
                1 => 'wooly',
            ],
        1728 =>
            [
                0 => 'worshipped ',
                1 => 'worshiped',
            ],
        1729 =>
            [
                0 => 'worshipping ',
                1 => 'worshiping ',
            ],
        1730 =>
            [
                0 => 'worshipper',
                1 => 'worshiper',
            ],
        1731 =>
            [
                0 => 'yodelled',
                1 => 'yodeled',
            ],
        1732 =>
            [
                0 => 'yodelling',
                1 => 'yodeling',
            ],
        1733 =>
            [
                0 => 'yoghourt',
                1 => 'yogurt',
            ],
        1734 =>
            [
                0 => 'yoghourts',
                1 => 'yogurts',
            ],
        1735 =>
            [
                0 => 'yoghurt',
                1 => 'yogurt',
            ],
        1736 =>
            [
                0 => 'yoghurts',
                1 => 'yogurts',
            ],
    ];

    public static function getSynonymsAsString()
    {
        $result = '';
        foreach (static::$uk_us as $key => $synonym) {
            $result .= $synonym[0] . ',' . $synonym[1] . "\n";
        }

        return $result;
    }
}